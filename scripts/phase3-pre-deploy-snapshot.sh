...
ALEMBIC_VERSION=$(docker compose -f docker-compose.yml exec -T postgres psql -U qlts -d "${POSTGRES_DB:-qlts_production}" -t -c \
...
docker compose -f docker-compose.yml exec -T postgres psql -U qlts -d "${POSTGRES_DB:-qlts_production}" -c "
...
docker compose -f docker-compose.yml exec -T postgres pg_dump -U qlts \
  --format=custom \
  --no-owner \
  --no-acl \
...
docker compose -f docker-compose.yml cp postgres:/tmp/snapshot.dump "${SNAPSHOT_PATH}"
docker compose -f docker-compose.yml exec -T postgres rm /tmp/snapshot.dump
...
echo "  docker compose -f docker-compose.yml exec postgres pg_restore -U qlts -d ${POSTGRES_DB:-qlts_production} ${SNAPSHOT_PATH}"
echo ""
