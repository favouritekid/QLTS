...
assert len(OFFICER_CAPACITIES) == NUM_OFFICERS, "OFFICER_CAPACITIES length must match NUM_OFFICERS"
...
StressSessionLocal = sessionmaker(
...
async def clear_existing_test_data(session: AsyncSession):
    ...
    officer_ids_to_clear = list(range(OFFICER_START_ID, OFFICER_START_ID + NUM_OFFICERS))
    ...
    await session.execute(
        delete(models.AssignmentLog).where(models.AssignmentLog.officer_id.in_(officer_ids_to_clear))
    )
    # Xóa log của các lead tạo bởi stress test cũ (nếu có cách xác định, ví dụ qua source)
    await session.execute(
        delete(models.AssignmentLog).where(models.AssignmentLog.lead.has(models.Lead.source == "stress_test"))
    )
    ...
    await session.execute(
        delete(models.Lead).where(models.Lead.assigned_officer_id.in_([102])) # ID cũ của officer_busy
    )
    ...
    await session.execute(
        delete(models.OrganizationUnit).where(models.OrganizationUnit.id.in_([UNIT_ID_TO_TEST, UNIT_ID_OTHER]))
    )

    # 5. Xóa Statuses & Stages (Cẩn thận hơn)
    await session.execute(
        delete(models.ConsultationStatus).where(models.ConsultationStatus.id.in_([
            INITIAL_STATUS_ID, ASSIGNED_STATUS_ID, UNASSIGNED_STATUS_ID, LOST_STATUS_ID
        ]))
    )
    await session.execute(
        delete(models.PipelineStage).where(models.PipelineStage.id.in_([STAGE_A_ID, STAGE_LOST_ID]))
    )
...
async def seed_database_for_stress_test():
    ...
    async with StressSessionLocal() as session:
        ...
        async with session.begin(): # Bắt đầu transaction mới cho việc seed
            ...
            # 1. Seed Units & Major
            unit1 = models.OrganizationUnit(id=UNIT_ID_TO_TEST, name="Worker Test Unit 1", type="Faculty")
            unit2 = models.OrganizationUnit(id=UNIT_ID_OTHER, name="Worker Test Unit 2", type="Faculty") # Vẫn tạo unit khác
            major1 = models.Major(id=MAJOR_ID_TO_TEST, name="Worker Test Major 1", code="WTM1", unit_id=UNIT_ID_TO_TEST)
            session.add_all([unit1, unit2, major1])

            # 2. Seed Stages & Statuses
            stage_a = models.PipelineStage(id=STAGE_A_ID, name="Worker Stage A", order=10)
            stage_lost = models.PipelineStage(id=STAGE_LOST_ID, name="Worker Stage Lost", order=99)
            session.add_all([stage_a, stage_lost])

            statuses_to_create = [
                models.ConsultationStatus(id=INITIAL_STATUS_ID, name="Initial Worker", color_code="#AAAAAA", stage_id=STAGE_A_ID),
                models.ConsultationStatus(id=ASSIGNED_STATUS_ID, name="Assigned Worker", color_code="#BBBBBB", stage_id=STAGE_A_ID),
                models.ConsultationStatus(id=UNASSIGNED_STATUS_ID, name="Unassigned Worker", color_code="#CCCCCC", stage_id=STAGE_A_ID),
                models.ConsultationStatus(id=LOST_STATUS_ID, name="Lost Worker", color_code="#DDDDDD", stage_id=STAGE_LOST_ID),
            ]
            ...
            officer_other = models.User(
                 id=104, username="worker_other", email="other@worker.com", unit_id=UNIT_ID_OTHER,
                 max_capacity=5, last_assigned_at=None, role="officer", status="active",
                 availability_status="available", password_hash=TEST_PASSWORD_HASH
            )
            session.add_all(officers_to_add)
            session.add(officer_other)
            log.info(f"Seeded {len(officers_to_add)} officers for Unit {UNIT_ID_TO_TEST} with capacities: {OFFICER_CAPACITIES}")
...
async def create_leads_in_db():
    ...
    async with StressSessionLocal() as session:
        async with session.begin():
            ...
            if not status_obj:
                log.error(f"FATAL: Initial status {INITIAL_STATUS_ID} not found after seeding.")
                return []
            ...
            tasks_to_dispatch = [lead.id for lead in leads_to_add if lead.id is not None]

        log.info(f"Created {len(tasks_to_dispatch)} leads. IDs from {tasks_to_dispatch[0]} to {tasks_to_dispatch[-1]}")
...
async def analyze_results():
    ...
    log.info(f"Waiting {RESULTS_ANALYSIS_DELAY_SECONDS} seconds for workers to process tasks...")
    ...
    async with StressSessionLocal() as session:
        ...
        total_leads_result = await session.execute(
            select(func.count(models.Lead.id)).where(models.Lead.source == "stress_test")
        )
        total_leads = total_leads_result.scalar_one_or_none() or 0

        if total_leads == 0:
            log.warning("No leads found with source 'stress_test'. Cannot analyze results.")
            return
        ...
        other_status_leads = total_leads - assigned_leads - unassigned_leads - initial_state_leads

        # Lấy số lượng lead được gán cho mỗi officer
        assignment_counts_result = await session.execute(
            select(models.Lead.assigned_officer_id, func.count(models.Lead.id).label("count"))
            .where(
                models.Lead.source == "stress_test",
        ...
        assignment_counts = {row.assigned_officer_id: row.count for row in assignment_counts_result}

    # In kết quả
    log.info("--- Stress Test Results ---")
    log.info(f"Total Leads Created: {total_leads}")
    log.info(f"Leads Assigned (Unit {UNIT_ID_TO_TEST}): {assigned_leads} ({assigned_leads/total_leads:.1%})")
    ...
    if assigned_leads >= target_assigned:
        log.info(f"✅ Assignment goal met ({assigned_leads}/{LEAD_COUNT} assigned >= {target_assigned})")
    else:
        log.warning(f"⚠️ Assignment goal NOT met ({assigned_leads}/{LEAD_COUNT} assigned < {target_assigned})")
        log.warning("   Consider increasing RESULTS_ANALYSIS_DELAY_SECONDS or checking worker logs for errors.")


async def main():
    if settings.APP_ENV != 'test':
        log.error(f"LỖI: APP_ENV không phải là 'test' (hiện tại là '{settings.APP_ENV}').")
    ...
    log.info(f"Lead Count: {LEAD_COUNT}, Num Officers: {NUM_OFFICERS}, Capacities: {OFFICER_CAPACITIES}")
