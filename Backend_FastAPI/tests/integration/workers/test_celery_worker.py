...
from sqlalchemy import func, select  # Import func
...
async def seed_worker_test_data(setup_test_database, admin_user_in_db):
    ...
    statuses_to_create = [
        ...
        {
            "id": unassigned_pending_status_id,
    ...
    valid_password_hash = admin_user_in_db.get(
        "password_hash", "$2b$12$placeholderhashforworker"
    )

    officer_idle_data = {
        "id": 101,
        "username": "worker_idle",
        "email": "idle@worker.com",
        "unit_id": unit1_data["id"],
        "max_capacity": 5,
    ...
    officer_busy_data = {
        ...
        "email": "busy@worker.com",
        "unit_id": unit1_data["id"],
        "max_capacity": 3,
        "last_assigned_at": datetime.now(timezone.utc) - timedelta(days=1),
    }
    officer_recent_data = {
        "id": 103,
        "username": "worker_recent",
        "email": "recent@worker.com",
        "unit_id": unit1_data["id"],
        "max_capacity": 5,
        "last_assigned_at": datetime.now(timezone.utc) - timedelta(hours=1),
    }
    officer_other_unit_data = {
        "id": 104,
        "username": "worker_other",
        "email": "other@worker.com",
        "unit_id": unit2_data["id"],
        "max_capacity": 5,
        "last_assigned_at": None,
    }
    ...
    async with AsyncSessionLocal() as session:
        async with session.begin():
            session.add_all(
                [
                    models.OrganizationUnit(**unit1_data),
                    models.OrganizationUnit(**unit2_data),
                ]
            )
            session.add(models.Major(**major1_data))
            session.add_all(
                [models.PipelineStage(**stage_a), models.PipelineStage(**stage_lost)]
            )
            session.add_all(
                [models.ConsultationStatus(**s) for s in statuses_to_create]
            )
            officers = [
                models.User(
                    **officer_idle_data,
            ...
            for i in range(officer_busy_data["max_capacity"]):
                busy_officer_leads.append(
                    models.Lead(
                        full_name=f"Busy Lead {i+1}",
                        email=f"busy{i}@lead.com",
                        phone=f"000{i}",
                        source="test",  # Phone ngắn
                        unit_id=unit1_data["id"],
                        status=assigned_status_id,
                        assigned_officer_id=officer_busy_data["id"],
                    )
                )
...
async def _create_lead_via_api(
    client: AsyncClient,
    headers: dict,
    unit_id: int,
    major_id: int,
    ...
    if response.status_code != 201:
        pytest.fail(
            f"Failed to create lead via API (Status: {response.status_code}): {response.text}"
        )
...
async def _wait_for_lead_assignment(
    lead_id: int,
    expected_officer_id: int = None,
    expected_status: str = None,
    timeout: int = 20,
) -> models.Lead | None:  # TĂNG TIMEOUT LÊN 20S
    """Chờ đợi (poll DB) cho đến khi Lead đạt trạng thái mong đợi."""
    start_time = asyncio.get_event_loop().time()
    log.info(
        f"Polling DB for Lead ID {lead_id} (Timeout: {timeout}s). Expecting Officer: {expected_officer_id}, Status: {expected_status}"
    )
    last_status = None
    ...
    while asyncio.get_event_loop().time() - start_time < timeout:
        ...
        async with AsyncSessionLocal() as session:
            ...
            if db_lead:
                ...
                officer_matches = (
                    (expected_officer_id is db_lead.assigned_officer_id)
                    if expected_officer_id is None
                    else (db_lead.assigned_officer_id == expected_officer_id)
                )
                status_matches = (
                    (expected_status is db_lead.status)
                    if expected_status is None
                    else (db_lead.status == expected_status)
                )

                if officer_matches and status_matches:
                    log.info(
                        f"Lead ID {lead_id} reached expected state (Officer: {db_lead.assigned_officer_id}, Status: {db_lead.status})."
                    )
                    return db_lead
            else:
                log.warning(
                    f"Lead ID {lead_id} not found during polling."
                )  # Có thể xảy ra nếu poll quá nhanh

    log.error(
        f"Timeout waiting for Lead ID {lead_id}. Last seen state: Officer={last_officer_id}, Status='{last_status}'. Expected: Officer={expected_officer_id}, Status='{expected_status}'."
    )
    return None


async def _check_assignment_log(
    lead_id: int, officer_id: int, method: str = "automatic"
):
    ...
    async with AsyncSessionLocal() as session:
        ...
        ), f"AssignmentLog not found for lead {lead_id}, officer {officer_id}, method {method}"
...
async def _check_officer_last_assigned(officer_id: int, check_is_recent: bool = True):
    ...
    async with AsyncSessionLocal() as session:
        ...
        assert (
            officer.last_assigned_at is not None
        ), f"Officer {officer_id} last_assigned_at should not be None"
        if check_is_recent:
            time_diff = datetime.now(timezone.utc) - officer.last_assigned_at
            # Tăng khoảng thời gian chấp nhận được lên, ví dụ 120 giây
            assert (
                time_diff.total_seconds() < 120
            ), f"Officer {officer_id} last_assigned_at is not recent (diff: {time_diff.total_seconds()}s)"
...
@pytest.mark.asyncio
async def test_celery_assign_success_least_busy(
    client: AsyncClient,
    admin_token_headers: dict,
    seed_worker_test_data: dict,
    test_redis_client,
):
    """(Giữ nguyên)"""
    ...
    lead_id = await _create_lead_via_api(
        client, admin_token_headers, unit_id, major_id, "_leastbusy"
    )
    assigned_lead = await _wait_for_lead_assignment(
        lead_id,
        expected_officer_id=expected_officer_id,
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_celery_assign_success_fairness(
    client: AsyncClient,
    admin_token_headers: dict,
    seed_worker_test_data: dict,
    test_redis_client,
    ...
    expected_status = seed_worker_test_data["assigned_status_id"]
    lead_id = await _create_lead_via_api(
        client, admin_token_headers, unit_id, major_id, "_fairness"
    )
    assigned_lead = await _wait_for_lead_assignment(
        lead_id,
        expected_officer_id=expected_officer_id,
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_celery_assign_officer_in_other_unit(
    client: AsyncClient,
    admin_token_headers: dict,
    seed_worker_test_data: dict,
    test_redis_client,
    ...
    lead_id = await _create_lead_via_api(
        client, admin_token_headers, unit_id, major_id, "_unit2officer"
    )

    # Wait & Verify
    updated_lead = await _wait_for_lead_assignment(
        lead_id,
        expected_officer_id=expected_officer_id,
        expected_status=expected_status,
    )
    ...
    await _check_officer_last_assigned(expected_officer_id)
    log.info(
        f"--- Finished: test_celery_assign_officer_in_other_unit (Assigned to {expected_officer_id}) ---"
    )


@pytest.mark.integration
@pytest.mark.asyncio
async def test_celery_assign_all_officers_full_capacity(
    client: AsyncClient,
    admin_token_headers: dict,
    seed_worker_test_data: dict,
    test_redis_client,
    ...
    log.info(
        f"Filling capacity for officers {idle_officer_id} and {recent_officer_id}..."
    )
    async with AsyncSessionLocal() as session:
        async with session.begin():
            leads_to_add = []
            for i in range(idle_capacity):
                leads_to_add.append(
                    models.Lead(
                        full_name=f"IF{i}",
                        email=f"if{i}@l.co",
                        ...
                        unit_id=unit_id,
                        status="assigned",
                        assigned_officer_id=idle_officer_id,
                    )
                )
            for i in range(recent_capacity):
                leads_to_add.append(
                    models.Lead(
                        ...
                        unit_id=unit_id,
                        status="assigned",
                        assigned_officer_id=recent_officer_id,
                    )
                )
    ...
    lead_id = await _create_lead_via_api(
        client, admin_token_headers, unit_id, major_id, "_fullcapacity"
    )
    updated_lead = await _wait_for_lead_assignment(
        lead_id, expected_status=expected_status
    )
    ...
    async with AsyncSessionLocal() as session:
        log_count = await session.scalar(
            select(func.count(models.AssignmentLog.id)).where(
                models.AssignmentLog.lead_id == lead_id
            )
        )
        assert log_count == 0
