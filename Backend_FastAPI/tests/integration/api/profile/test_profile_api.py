...
from app.security import (
    verify_password,  # Có thể cần nếu test đổi pass qua profile (hiện tại không có)
)
...
async def test_get_profile_success(
    ...
    response = await client.get(
        ProfileURLs.PROFILE, headers=regular_user_token_headers
    )  # Dùng constant URL
    ...
    assert isinstance(
        profile_data, dict
    ), "Response body should be a dictionary (User object)"

    # 1. Kiểm tra các trường dữ liệu chính khớp với fixture
    assert (
        profile_data.get("username") == regular_user_in_db["username"]
    ), "Username mismatch"
    ...
    assert (
        profile_data.get("role") == TestUsers.REGULAR["role"]
    ), "Role mismatch"  # Kiểm tra role từ constant
    assert (
        profile_data.get("status") == TestUsers.REGULAR["status"]
    ), "Status mismatch"  # Kiểm tra status

    # 2. Kiểm tra các trường có thể null ban đầu (tùy thuộc fixture)
    assert (
        profile_data.get("full_name") is None
    ), "Expected initial full_name to be None"
    assert (
        profile_data.get("phone_number") is None
    ), "Expected initial phone_number to be None"
    assert (
        profile_data.get("avatar_url") is None
    ), "Expected initial avatar_url to be None"

    # 3. Đảm bảo thông tin nhạy cảm không bị rò rỉ
    assert (
        "password_hash" not in profile_data
    ), "Password hash leaked in profile response"
...
async def test_update_profile_success(
    ...
    response = await client.put(
        ProfileURLs.PROFILE, data=update_data_form, headers=regular_user_token_headers
    )
...
async def test_update_profile_duplicate_email(
    client: AsyncClient, admin_user_in_db: dict, regular_user_token_headers: dict
):
    ...
    update_data_form = {"email": duplicate_email}
    response = await client.put(
        ProfileURLs.PROFILE, data=update_data_form, headers=regular_user_token_headers
    )
...
async def test_update_profile_invalid_email(
    ...
    update_data_form = {"email": invalid_email_str}
    response = await client.put(
        ProfileURLs.PROFILE, data=update_data_form, headers=regular_user_token_headers
    )
    ...
    ), f"Invalid Email Resp: {response.text}"  # Unprocessable Entity
    error_data = response.json()
    ...
    ), "'detail' should be a string for this error type"

    # Kiểm tra message lỗi cụ thể
    assert f"Invalid email format: {invalid_email_str}" in error_data["detail"]
    assert (
        "value is not a valid email address" in error_data["detail"]
    )  # Kiểm tra phần lỗi từ Pydantic/EmailStr
    ...
    log.info(
        "Invalid email format correctly blocked (422) with detailed error in 'detail' field."
    )
    log.info("--- Finished: test_update_profile_invalid_email ---")


@pytest.mark.asyncio
async def test_update_profile_only_some_fields(
    client: AsyncClient, regular_user_token_headers: dict, regular_user_in_db: dict
):
    ...
    update_data_form = {"phone_number": "555-1234"}  # Chỉ cập nhật số điện thoại
    response = await client.put(
        ProfileURLs.PROFILE, data=update_data_form, headers=regular_user_token_headers
    )
...
async def test_update_profile_with_avatar_success(
    ...
    assert (
        updated_profile.get("avatar_url") == mock_avatar_url
    ), "Avatar URL không khớp trong response"
    ...
    async with AsyncSessionLocal() as session:
        ...
        assert db_user is not None
        assert (
            db_user.avatar_url == mock_avatar_url
        ), "Avatar URL không được cập nhật đúng trong DB"
        assert db_user.full_name == update_data_form["full_name"]
...
async def test_update_profile_avatar_bad_file_type(
    ...
    log.info(
        "Update profile with bad file type correctly failed (400) with specific message."
    )
