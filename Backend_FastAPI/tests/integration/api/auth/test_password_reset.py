...
from datetime import datetime, timedelta, timezone  # Added datetime here
...
from app.security import (  # Import security functions
...
async def test_forgot_password_success(
    ...
    mock_user_obj = User(
        id=regular_user_in_db["id"], email=user_email, username=user_username
    )
...
async def test_forgot_password_email_not_found(
    mock_celery_delay: AsyncMock, mock_get_user: AsyncMock, client: AsyncClient
):
    ...
    response = await client.post(
        AuthURLs.FORGOT_PASSWORD, json={"email": non_existent_email}
    )

    # --- Assert Response ---
    assert (
        response.status_code == 202
    ), f"Forgot Password (Not Found) Resp: {response.text}"
    resp_data = response.json()
    assert "msg" in resp_data
...
async def test_forgot_password_invalid_email_format(client: AsyncClient):
    ...
    # --- Action ---
    response = await client.post(
        AuthURLs.FORGOT_PASSWORD, json={"email": invalid_email}
    )
    ...
    for err in error_data["errors"]:
        if err.get(
            "field"
        ) == "body -> email" and "value is not a valid email address" in err.get(
            "message", ""
        ):
            found_error = True
            break
...
async def test_reset_password_invalid_token_string(client: AsyncClient):
    ...
    ), f"Invalid Token Resp: {response.text}"  # Handler ném InvalidToken (401)
    error_data = response.json()
    assert "detail" in error_data
    assert (
        "Could not validate credentials" in error_data["detail"]
    )  # Message từ InvalidToken handler
...
async def test_reset_password_expired_token(
    ...
    with patch("app.security.datetime") as mock_dt:
        mock_dt.now.return_value = datetime.now(timezone.utc) - timedelta(
            hours=1
        )  # Giả lập thời gian là 1h trước
        expired_token_negative_delta = create_password_reset_token(email=user_email)

    payload = {"token": expired_token_negative_delta, "new_password": new_password}
    response = await client.post(AuthURLs.RESET_PASSWORD, json=payload)
    ...
    assert (
        "Could not validate credentials" in error_data["detail"]
    )  # Lỗi do verify_password_reset_token trả về None
    log.info("Expired token correctly blocked (401).")


@pytest.mark.asyncio
async def test_reset_password_user_not_found_for_token(
    client: AsyncClient, regular_user_in_db: dict
):
    ...
    assert "detail" in error_data
    assert (
        "User associated with this token not found" in error_data["detail"]
    )  # Message từ service
...
async def test_reset_password_weak_password(
    ...
    payload = {"token": valid_token, "new_password": weak_password}
    response = await client.post(AuthURLs.RESET_PASSWORD, json=payload)
    ...
    for err in error_data["errors"]:
        if err.get("field") == "body -> new_password":
            found_error = True
            assert "at least 8 characters" in err.get(
                "message", ""
            ) or "Password must contain" in err.get("message", "")
            break
