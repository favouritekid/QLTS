...
Wave 2 (2026-04-21): source of truth migrated from the deprecated
...
# All 12 Phase 1 events that are user-class in `EVENT_CATALOG`.
# LEAD_UPDATED is intentionally excluded: catalog classifies it as
# `broadcast_only` (UI realtime sync, no admin-configurable contract),
# so it does not carry condition_fields. The deprecated
# `EVENT_METADATA_REGISTRY` did expose condition_fields for it, but
# catalog is the current source of truth — see
# `app/core/event_catalog.py` LEAD_UPDATED entry.
PHASE1_EVENTS = [
...
OPERATOR_SETS_BY_TYPE = {
    "string": set(_OPS_STR),
    "integer": set(_OPS_NUM),
    "float": set(_OPS_NUM),
    "datetime": set(_OPS_NUM),
    "boolean": set(_OPS_BOOL),
    "array": set(_OPS_ARR),
}
...
class TestConditionFieldsExist:
    ...
    def test_all_phase1_events_have_condition_fields(self):
        ...
        for event in PHASE1_EVENTS:
            ...
            assert meta is not None, f"{event.value} not in catalog"
...
class TestLeadImportedNoLeadFields:
    ...
    def test_no_lead_namespace(self):
        meta = EVENT_CATALOG[SystemEvents.LEAD_IMPORTED]
...
class TestConsultationReminderNoActor:
    ...
    def test_no_actor_namespace(self):
        meta = EVENT_CATALOG[SystemEvents.CONSULTATION_REMINDER]
...
class TestLeadStatusChangedHasEventFields:
    ...
    def test_has_status_fields(self):
        meta = EVENT_CATALOG[SystemEvents.LEAD_STATUS_CHANGED]
        paths = {cf.path for cf in meta.condition_fields}
