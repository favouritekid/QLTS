...
from app.models import PipelineStage, ConsultationStatus, Lead # Thêm Lead để test ràng buộc
...
STAGE_LIST_DICTS_SORTED = sorted([TestPipelineData.STAGE_A, TestPipelineData.STAGE_B], key=lambda x: x['order'])
...
STATUS_LIST_DICTS_SORTED_BY_ID = sorted(STATUS_LIST_DICTS_UNSORTED, key=lambda x: x['id'])


# === Tests cho get_all_pipeline_stages ===

@pytest.mark.asyncio
@patch('app.services.pipeline_service.safe_redis_get', new_callable=AsyncMock, return_value=None)
...
async def test_get_stages_cache_miss(mock_redis_set, mock_redis_get, mock_db_session):
    ...
    mock_db_session.execute.return_value.scalars.return_value.all.return_value = STAGE_LIST_MODELS
...
@pytest.mark.asyncio
@patch('app.services.pipeline_service.safe_redis_get', new_callable=AsyncMock, side_effect=redis.RedisError("Get failed"))
@patch('app.services.pipeline_service.safe_redis_set', new_callable=AsyncMock)
@patch('app.services.pipeline_service.log.error', new_callable=AsyncMock) # Mock logger
async def test_get_stages_redis_get_error(mock_log_error, mock_redis_set, mock_redis_get, mock_db_session):
...
@pytest.mark.asyncio
@patch('app.services.pipeline_service.safe_redis_get', new_callable=AsyncMock, return_value=None)
@patch('app.services.pipeline_service.safe_redis_set', new_callable=AsyncMock, side_effect=redis.RedisError("Set failed"))
@patch('app.services.pipeline_service.log.error', new_callable=AsyncMock) # Mock logger
async def test_get_stages_redis_set_error(mock_log_error, mock_redis_set, mock_redis_get, mock_db_session):
...
async def test_get_statuses_cache_miss(mock_redis_set, mock_redis_get, mock_db_session):
    ...
    mock_db_session.execute.return_value.scalars.return_value.all.return_value = STATUS_LIST_MODELS
...
async def test_get_statuses_cache_hit(mock_redis_set, mock_redis_get, mock_db_session):
    ...
    cached_data = [{"id": "CACHED_T", "name": "Cached Status", "color_code": "#ccc", "stage_id": "S1"}]
    mock_redis_get.return_value = json.dumps(cached_data)
    result = await pipeline_service.get_all_consultation_statuses(mock_db_session)
    assert result == cached_data
    mock_redis_get.assert_awaited_once_with(STATUSES_KEY)
...
@pytest.mark.asyncio
@patch('app.services.pipeline_service.safe_redis_delete', new_callable=AsyncMock, side_effect=redis.RedisError("Delete failed"))
@patch('app.services.pipeline_service.log.error', new_callable=AsyncMock) # Mock logger
...
async def test_create_pipeline_stage_success(mock_invalidate, mock_db_session):
    ...
    created_stage = await pipeline_service.create_pipeline_stage(mock_db_session, stage_in)
...
async def test_create_pipeline_stage_duplicate_id(mock_db_session):
    ...
    assert exc_info.value.detail == f"Pipeline Stage ID '{stage_data['id']}' already exists."
...
async def test_create_pipeline_stage_duplicate_order(mock_db_session):
    ...
    stage_in = schemas.PipelineStageCreate(**stage_data)
    mock_db_session.get.return_value = None
    mock_db_session.scalar.return_value = PipelineStage(id="OTHER_ID", name="Other", order=10)
    with pytest.raises(DuplicateResourceError) as exc_info:
        await pipeline_service.create_pipeline_stage(mock_db_session, stage_in)
    assert exc_info.value.detail == f"Pipeline Stage order '{stage_data['order']}' already exists."
    mock_db_session.add.assert_not_called()
    mock_db_session.commit.assert_not_awaited()
...
async def test_update_pipeline_stage_success(mock_invalidate, mock_db_session):
    ...
    mock_db_session.scalar.return_value = None
    updated_stage = await pipeline_service.update_pipeline_stage(mock_db_session, stage_id, update_in)
...
async def test_update_pipeline_stage_duplicate_order(mock_db_session):
    ...
    with pytest.raises(DuplicateResourceError) as exc_info:
        await pipeline_service.update_pipeline_stage(mock_db_session, stage_id, update_in)
    assert exc_info.value.detail == f"Pipeline Stage order '{STAGE_A_MODEL.order}' already in use."
    mock_db_session.add.assert_not_called()
    mock_db_session.commit.assert_not_awaited()

@pytest.mark.asyncio
@patch('app.services.pipeline_service.invalidate_pipeline_cache', new_callable=AsyncMock) # Thêm mock invalidate
async def test_delete_pipeline_stage_success(mock_invalidate, mock_db_session): # Thêm mock_invalidate
...
async def test_delete_pipeline_stage_in_use(mock_db_session):
    ...
    assert "1 consultation statuses linked to it" in exc_info.value.detail # Sửa số lượng nếu cần
...
async def test_create_consultation_status_success(mock_invalidate, mock_db_session):
    ...
    status_data = {"id": "NEW_T", "name": "New Status", "color_code": "#123456", "stage_id": STAGE_A_MODEL.id}
    status_in = schemas.ConsultationStatusCreate(**status_data)
    mock_db_session.get.side_effect = [None, STAGE_A_MODEL]
    created_status = await pipeline_service.create_consultation_status(mock_db_session, status_in)
...
async def test_create_consultation_status_duplicate_id(mock_db_session):
    ...
    assert exc_info.value.detail == f"Consultation Status ID '{status_data['id']}' already exists."
    mock_db_session.add.assert_not_called()
...
async def test_create_consultation_status_stage_not_found(mock_db_session):
    ...
    status_data = {"id": "NEW_T", "name": "New", "color_code": "#ffffff", "stage_id": invalid_stage_id}
    status_in = schemas.ConsultationStatusCreate(**status_data)
...
@pytest.mark.asyncio
@patch('app.services.pipeline_service.invalidate_pipeline_cache', new_callable=AsyncMock) # Thêm mock invalidate
async def test_delete_consultation_status_success(mock_invalidate, mock_db_session): # Thêm mock_invalidate
    """Test delete_consultation_status: Xóa thành công."""
    status_id = "STATUS_TO_DELETE"
    status_to_delete = ConsultationStatus(id=status_id, name="Delete Me", color_code="#eee", stage_id=STAGE_A_MODEL.id)
    mock_db_session.get.return_value = status_to_delete
    # Mock scalar (check lead count và consultation count) trả về 0
    mock_db_session.scalar.side_effect = [0, 0] # Lần 1 cho lead, lần 2 cho consultation
