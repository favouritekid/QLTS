...
REAL_PASSWORD_HASH_FOR_TESTUSER = "$2b$12$BvEj6Ldp4bfmCgqAikhEa.QE9TCNY7scpcDtvlJDGB4se.7rSZQdS" 
...
async def test_authenticate_user_invalid_password(mocker):
    ...
    mock_get_user.assert_awaited_once_with(mocker.ANY, TEST_USERNAME)
    mock_verify.assert_called_once_with(INVALID_PASSWORD, REAL_PASSWORD_HASH_FOR_TESTUSER)
...
async def test_authenticate_user_not_found(mocker):
    ...
    logging.info(f"[{test_name}] Calling authenticate_user with non-existent username...")
    with pytest.raises(InvalidCredentials) as exc_info:
        await user_service.authenticate_user(
            db=AsyncMock(),
    ...
    mock_verify.assert_called_once_with("anypassword", DUMMY_BCRYPT_HASH) # <-- Sử dụng hằng số
    logging.info(f"[{test_name}] Mock call assertions successful (verify called with dummy hash).")
    logging.info(f"--- Finished test: {test_name} ---")