...
# Import constants
try:
    from ..fixtures.constants import TestOrgData, NON_EXISTENT_ID
except ImportError:
...
def mock_unit_2_child(mock_unit_1):
    ...
    unit = models.OrganizationUnit(id=2, name="Test Unit 2 Child", type="Department", parent_id=mock_unit_1.id)
...
def mock_internal_get_unit(mocker):
    ...
    return mocker.patch('app.services.organization_service.get_organization_unit_by_id', new_callable=AsyncMock)

@pytest.fixture
def mock_internal_get_major(mocker):
    """Mocks the service's internal call to get_major_by_id"""
    return mocker.patch('app.services.organization_service.get_major_by_id', new_callable=AsyncMock)
...
@pytest.mark.asyncio
async def test_get_all_organization_units(mock_db_session, mock_unit_1, mock_unit_2_child):
    """Test lấy tất cả units thành công."""
    mock_list = [mock_unit_1, mock_unit_2_child]
    mock_db_session.execute.return_value.scalars.return_value.unique.return_value.all.return_value = mock_list
...
async def test_get_organization_unit_by_id_found(mock_db_session, mock_unit_1):
    ...
    mock_db_session.execute.return_value.scalars.return_value.unique.return_value.one_or_none.return_value = mock_unit_1
    
    result = await organization_service.get_organization_unit_by_id(mock_db_session, mock_unit_1.id)
...
async def test_get_organization_unit_by_id_not_found(mock_db_session):
    ...
    mock_db_session.execute.return_value.scalars.return_value.unique.return_value.one_or_none.return_value = None
    
    with pytest.raises(ResourceNotFoundError) as exc_info:
        await organization_service.get_organization_unit_by_id(mock_db_session, NON_EXISTENT_ID)
    
    assert str(NON_EXISTENT_ID) in exc_info.value.detail

@pytest.mark.asyncio
async def test_create_organization_unit_success(mock_db_session, mock_internal_get_unit, mock_unit_1):
    """Test tạo unit thành công (không có parent)."""
    unit_schema = schemas.OrganizationUnitCreate(name="New Unit", type="Faculty", parent_id=None)
    ...
    result = await organization_service.create_organization_unit(mock_db_session, unit_schema)
...
@pytest.mark.asyncio
async def test_create_organization_unit_parent_not_found(mock_db_session, mock_internal_get_unit):
    """Test tạo unit thất bại (parent_id không tồn tại)."""
    unit_schema = schemas.OrganizationUnitCreate(name="New Unit", type="Faculty", parent_id=NON_EXISTENT_ID)
    
    mock_db_session.get.return_value = None # Không tìm thấy parent
    
    with pytest.raises(ResourceNotFoundError) as exc_info:
        await organization_service.create_organization_unit(mock_db_session, unit_schema)
...
@pytest.mark.asyncio
async def test_update_organization_unit_success(mock_db_session, mock_internal_get_unit, mock_unit_1, mock_unit_2_child):
    """Test cập nhật unit thành công."""
    unit_id = mock_unit_1.id
    update_schema = schemas.OrganizationUnitUpdate(name="Updated Name", parent_id=mock_unit_2_child.id)
    ...
    result = await organization_service.update_organization_unit(mock_db_session, unit_id, update_schema)
    
    assert mock_internal_get_unit.await_count == 2
    mock_db_session.get.assert_awaited_once_with(models.OrganizationUnit, mock_unit_2_child.id)
...
@pytest.mark.asyncio
async def test_update_organization_unit_parent_self(mock_db_session, mock_internal_get_unit, mock_unit_1):
    ...
    with pytest.raises(DuplicateResourceError) as exc_info:
        await organization_service.update_organization_unit(mock_db_session, unit_id, update_schema)
        
    assert "A unit cannot be its own parent" in exc_info.value.detail
    mock_db_session.rollback.assert_awaited_once()

@pytest.mark.asyncio
async def test_delete_organization_unit_success(mock_db_session, mock_internal_get_unit, mock_unit_1):
...
@pytest.mark.asyncio
async def test_delete_organization_unit_conflict_children(mock_db_session, mock_internal_get_unit, mock_unit_1, mock_unit_2_child):
...
async def test_get_major_by_id_found(mock_db_session, mock_major_1):
    ...
    result = await organization_service.get_major_by_id(mock_db_session, mock_major_1.id)
...
async def test_create_major_success(mock_db_session, mock_major_1):
    ...
    major_schema = schemas.MajorCreate(
        name=mock_major_1.name, 
        code=mock_major_1.code, 
        unit_id=mock_major_1.unit_id
    )
    mock_db_session.execute.return_value.scalar_one_or_none.return_value = None # Không trùng code
...
async def test_create_major_duplicate_code(mock_db_session, mock_major_1):
    ...
    mock_db_session.execute.return_value.scalar_one_or_none.return_value = mock_major_1 # Trùng code
    
    with pytest.raises(DuplicateResourceError) as exc_info:
        await organization_service.create_major(mock_db_session, major_schema)
        
    assert f"Major with code '{major_schema.code}' already exists" in exc_info.value.detail
    mock_db_session.rollback.assert_awaited_once()
...
async def test_get_majors_by_unit_tree_success(mock_db_session, mock_major_1):
    ...
    result = await organization_service.get_majors_by_unit_tree(mock_db_session, unit_id)
...
async def test_get_majors_by_unit_tree_with_search(mock_db_session, mock_major_1):
    ...
    result = await organization_service.get_majors_by_unit_tree(mock_db_session, unit_id, search_term)
