...
from app.models import OfficerAssignmentConfig, SkillRequirementRule, OrganizationUnit
...
# Import constants
try:
    from ..fixtures.constants import TestOrgData, TestConfigData
except ImportError:
...
ASSIGNMENT_CONFIG_MODEL = OfficerAssignmentConfig(id=1, unit_id=UNIT_ID, params=ASSIGNMENT_PARAMS)
...
@patch('app.services.config_service.log.debug', new_callable=AsyncMock) # Mock logger debug
async def test_get_assignment_config_cache_miss(mock_log_debug, mock_redis_set, mock_redis_get, mock_db_session):
...
async def test_get_assignment_config_cache_hit(mock_log_debug, mock_redis_set, mock_redis_get, mock_db_session):
...
@patch('app.services.config_service.safe_redis_set', new_callable=AsyncMock)
@patch('app.services.config_service.log.error', new_callable=AsyncMock) # Mock logger error
async def test_get_assignment_config_redis_get_error(mock_log_error, mock_redis_set, mock_redis_get, mock_db_session):
...
@patch('app.services.config_service.log.error', new_callable=AsyncMock) # Mock logger error
async def test_get_assignment_config_redis_set_error(mock_log_error, mock_redis_set, mock_redis_get, mock_db_session):
...
@patch('app.services.config_service.log.info', new_callable=AsyncMock) # Mock logger info
async def test_update_config_create_success(mock_log_info, mock_redis_delete, mock_db_session):
    ...
    result_model = await config_service.update_assignment_config(mock_db_session, UNIT_ID, new_params)
    mock_db_session.scalar.assert_awaited_once()
    ...
    mock_db_session.refresh.assert_awaited_once_with(added_arg, attribute_names=['params'])
    ...
    mock_log_info.assert_any_await("Invalidated assignment config cache", unit_id=UNIT_ID)


@pytest.mark.asyncio
@patch('app.services.config_service.safe_redis_delete', new_callable=AsyncMock)
@patch('app.services.config_service.log.info', new_callable=AsyncMock)
async def test_update_config_update_success(mock_log_info, mock_redis_delete, mock_db_session):
    ...
    result_model = await config_service.update_assignment_config(mock_db_session, UNIT_ID, updated_params)
    mock_db_session.scalar.assert_awaited_once()
    ...
    mock_db_session.commit.assert_awaited_once()
    mock_db_session.refresh.assert_awaited_once_with(existing_config, attribute_names=['params'])
    mock_redis_delete.assert_awaited_once_with(ASSIGNMENT_CACHE_KEY)
    assert result_model == existing_config
    mock_log_info.assert_any_await("Updating existing assignment config", unit_id=UNIT_ID)
    # --- Assert log message invalidate cache ---
    mock_log_info.assert_any_await("Invalidated assignment config cache", unit_id=UNIT_ID)
...
async def test_update_config_unit_not_found(mock_log_error, mock_db_session):
    ...
    with pytest.raises(ResourceNotFoundError) as exc_info:
        await config_service.update_assignment_config(mock_db_session, UNIT_ID, {"a": 1})
...
async def test_update_config_db_commit_error(mock_log_error, mock_redis_delete, mock_db_session):
...
@patch('app.services.config_service.log.error', new_callable=AsyncMock) # Mock logger error
async def test_update_config_redis_delete_error(mock_log_error, mock_redis_delete, mock_db_session):
    """Test update_assignment_config: DB thành công, lỗi khi xóa cache."""
    mock_db_session.scalar.return_value = None
    mock_db_session.get.return_value = UNIT_MODEL
    result_model = await config_service.update_assignment_config(mock_db_session, UNIT_ID, {"a": 1})
    mock_db_session.commit.assert_awaited_once()
...
async def test_get_all_skill_rules_success(mock_db_session):
    ...
    mock_rules = [SKILL_RULE_MODEL_1, SkillRequirementRule(id=2, lead_attribute="loc", attribute_value="hcm", required_skill="SalesHCM")]
    mock_db_session.execute.return_value.scalars.return_value.all.return_value = mock_rules
...
async def test_create_skill_rule_success(mock_db_session):
    ...
    result_rule = await config_service.create_skill_rule(mock_db_session, SKILL_RULE_CREATE_SCHEMA)
    mock_db_session.add.assert_called_once()
    added_arg = mock_db_session.add.call_args[0][0]
...
async def test_delete_skill_rule_success(mock_db_session):
    ...
    mock_db_session.get.assert_awaited_once_with(SkillRequirementRule, rule_id_to_delete)
