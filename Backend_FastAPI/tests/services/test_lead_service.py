...
from app.utils.exceptions import (
    ResourceNotFoundError, DuplicateResourceError, PermissionDeniedError, BadRequest
)

# Import constants and fixtures if needed (adjust paths as necessary)
try:
    from ..fixtures.constants import TestUsers, TestOrgData, TestPipelineData, TestLeadData, NON_EXISTENT_ID
except ImportError:
...
mock_initial_status = models.ConsultationStatus(id=settings.DEFAULT_INITIAL_LEAD_STATUS_ID, name="Initial", color_code="#AAAAAA", stage_id="STAGE_A")
mock_assigned_status = models.ConsultationStatus(id=settings.DEFAULT_ASSIGNED_LEAD_STATUS, name="Assigned", color_code="#BBBBBB", stage_id="STAGE_A")
mock_lost_status = models.ConsultationStatus(id=settings.DEFAULT_LOST_LEAD_STATUS_ID, name="Lost", color_code="#FF0000", stage_id="STAGE_LOST")
mock_consult_status = models.ConsultationStatus(id="TTHV001", name="Consulting", color_code="#0000FF", stage_id="STAGE_A")
...
def base_lead_assigned():
    ...
    base_lead_1_data = {
        k: v for k, v in TestLeadData.LEAD_1.items()
        if k not in ['status', 'consultation_status_id', 'pipeline_stage_id', 'assigned_officer_id']
    }
    return models.Lead(
        id=2, # ID đã có
        **base_lead_1_data, 
...
def mock_external_deps(mocker):
    ...
    mock_celery = mocker.patch('app.celery_utils.process_automatic_lead_assignment_task.delay', return_value=None)
    
    mock_get_lead_by_id = mocker.patch('app.services.lead_service.get_lead_by_id', new_callable=AsyncMock) 
...
@pytest.mark.asyncio
async def test_create_lead_success(mock_db_session, mock_external_deps, base_lead_unassigned):
    """Test POST /leads - Tạo thành công, kiểm tra status mặc định và side effect."""
    lead_in = schemas.LeadCreate(**mock_lead_data)
    
    # Mock DB calls
    mock_db_session.execute.return_value.scalar_one_or_none.return_value = None # Không tìm thấy duplicate
    ...
    mock_db_session.flush.assert_awaited_once_with([added_lead]) # Kiểm tra flush đã được gọi với Lead
...
@pytest.mark.asyncio
async def test_create_lead_duplicate_email_unit(mock_db_session, mock_external_deps, base_lead_unassigned):
...
@pytest.mark.asyncio
async def test_update_lead_success(mock_db_session, mock_external_deps, base_lead_unassigned):
    ...
    update_data = {"full_name": "Updated Lead Name", "consultation_status_id": mock_consult_status.id}
    ...
    updated_lead = await lead_service.update_lead(mock_db_session, lead_id, lead_update_schema, mock_admin_user)
...
@pytest.mark.asyncio
async def test_update_lead_duplicate_email(mock_db_session, mock_external_deps, base_lead_unassigned):
    """Test PUT /leads/{id} - Lỗi 409 khi cập nhật email trùng với user khác."""
    lead_id = base_lead_unassigned.id
    lead_to_update = base_lead_unassigned
    existing_other_lead = models.Lead(id=99, email="existing@example.com", unit_id=lead_to_update.unit_id)
    ...
    mock_db_session.execute.side_effect = [mock_find_lead_result, mock_duplicate_check_result]

    with pytest.raises(DuplicateResourceError) as exc_info:
        await lead_service.update_lead(mock_db_session, lead_id, lead_update_schema, mock_admin_user)

    assert exc_info.value.detail == "Another lead with this email already exists in the unit."

# === Tests for add_consultation ===

@pytest.mark.asyncio
async def test_add_consultation_success(mock_db_session, mock_external_deps, base_lead_assigned):
    ...
    consult_data = {
        "method": "call", "notes": "Good call", "outcome": "follow-up", "status_id": mock_consult_status.id
    }
    ...
    new_consultation = await lead_service.add_consultation(mock_db_session, lead_id, officer_id, consult_in)
...
@pytest.mark.asyncio
async def test_add_consultation_permission_denied(mock_db_session, mock_external_deps, base_lead_assigned):
    """Test POST /consultations - Lỗi 403 khi officer không được gán."""
    lead_id = base_lead_assigned.id
    officer_id = mock_other_officer.id 
    consult_in = schemas.ConsultationCreate(method="call", notes="test", status_id=mock_consult_status.id)
    ...
    with pytest.raises(PermissionDeniedError) as exc_info:
        await lead_service.add_consultation(mock_db_session, lead_id, officer_id, consult_in)

    assert exc_info.value.detail == "You are not assigned to this lead."
    # Đảm bảo get_lead_by_id được gọi
    mock_external_deps["get_lead_by_id"].assert_awaited_once_with(mock_db_session, lead_id)
...
@pytest.mark.asyncio
async def test_assign_lead_manually_success(mock_db_session, mock_external_deps, base_lead_unassigned):
    ...
    updated_lead = await lead_service.assign_lead_manually(mock_db_session, lead_id, officer_id, assigner_user)
...
@pytest.mark.asyncio
async def test_process_officer_action_reassign_success(mock_db_session, mock_external_deps, base_lead_assigned):
    ...
    updated_lead = await lead_service.process_officer_action(mock_db_session, lead_id, officer_user, action, reason)
...
@pytest.mark.asyncio
async def test_process_officer_action_reject_success(mock_db_session, mock_external_deps, base_lead_assigned):
...
@pytest.mark.asyncio
async def test_process_officer_action_permission_denied(mock_db_session, mock_external_deps, base_lead_assigned):
    ...
    with pytest.raises(PermissionDeniedError) as exc_info:
        await lead_service.process_officer_action(mock_db_session, lead_id, officer_user, "reassign", "Test")
...
@pytest.mark.asyncio
async def test_delete_consultation_permission_denied_officer(mock_db_session, mock_external_deps, base_lead_assigned):
    """Test DELETE /consultation - Lỗi 403 cho Officer."""
    lead_id = base_lead_assigned.id
    consult_id = 10
    mock_consultation = models.Consultation(id=consult_id, lead_id=lead_id, officer_id=mock_officer_user.id)
    ...
    with pytest.raises(PermissionDeniedError) as exc_info:
        await lead_service.delete_consultation(mock_db_session, lead_id, consult_id, mock_officer_user)
...
@pytest.mark.asyncio
async def test_revert_last_status_success(mock_db_session, mock_external_deps, base_lead_assigned):
    ...
    old_state = {"status": "TTHV000", "consultation_status_id": "TTHV000", "pipeline_stage_id": "STAGE_A", "assigned_officer_id": None}
    new_state = {"status": mock_assigned_lead.status, "consultation_status_id": mock_assigned_lead.consultation_status_id, "pipeline_stage_id": mock_assigned_lead.pipeline_stage_id, "assigned_officer_id": mock_assigned_lead.assigned_officer_id}
    mock_last_history = models.LeadStatusHistory(
        id=1, lead_id=lead_id,
        old_status=old_state["status"], new_status=new_state["status"],
        old_consultation_status_id=old_state["consultation_status_id"], new_consultation_status_id=new_state["consultation_status_id"],
        old_pipeline_stage_id=old_state["pipeline_stage_id"], new_pipeline_stage_id=new_state["pipeline_stage_id"],
        old_assigned_officer_id=old_state["assigned_officer_id"], new_assigned_officer_id=new_state["assigned_officer_id"],
        changed_at=datetime.now(timezone.utc) - timedelta(hours=1)
    )
    mock_db_session.scalar.return_value = mock_last_history # Mock cho query tìm history
    ...
    reverted_lead = await lead_service.revert_last_status(mock_db_session, lead_id, mock_admin_user, "Test revert")
