...
from sqlalchemy import func, case, select # Import cho test engagement score

# Sample Data
mock_lead = models.Lead(
    id=1, full_name="Insight Lead", email="i@e.com", phone="123", source="website",
    unit_id=1, status="contacted", gpa=7.5, education_level="Tốt nghiệp THPT",
    location="Hà Nội", officer_rating=4, officer_summary="Good fit",
...
status_c1 = models.ConsultationStatus(id="C1", name="Consulted 1", color_code="#111", stage_id="S1")
status_c2 = models.ConsultationStatus(id="C2", name="Consulted 2", color_code="#222", stage_id="S2")
status_c3 = models.ConsultationStatus(id="C3", name="Consulted 3", color_code="#333", stage_id="S2")
...
mock_consultations_data = [
    ...
    {"id": 1, "lead_id": 1, "method": "meeting", "outcome": "successful", "duration_minutes": 35, "consultation_date": datetime.now(timezone.utc) - timedelta(days=2), "officer_id": 1, "consultation_status_id": status_c1.id},
    # Follow-up call (medium score)
    {"id": 2, "lead_id": 1, "method": "call", "outcome": "follow-up", "duration_minutes": 12, "consultation_date": datetime.now(timezone.utc) - timedelta(days=10), "officer_id": 1, "consultation_status_id": status_c2.id},
    # Failed email (low/negative score)
    {"id": 3, "lead_id": 1, "method": "email", "outcome": "failed", "duration_minutes": None, "consultation_date": datetime.now(timezone.utc) - timedelta(days=20), "officer_id": 1, "consultation_status_id": status_c3.id},
]

# FIXED: Sử dụng dictionary cơ sở + truyền đối tượng quan hệ (consultation_status)
mock_timeline = [ 
    {"type": "consultation", "timestamp": mock_consultations_data[2]["consultation_date"], 
     "data": models.Consultation(**mock_consultations_data[2], consultation_status=status_c3)},
    {"type": "consultation", "timestamp": mock_consultations_data[1]["consultation_date"], 
     "data": models.Consultation(**mock_consultations_data[1], consultation_status=status_c2)},
    {"type": "consultation", "timestamp": mock_consultations_data[0]["consultation_date"], 
     "data": models.Consultation(**mock_consultations_data[0], consultation_status=status_c1)},
]
...
def mock_db_session():
    ...
    expected_outcome_score = cfg["outcome"]["successful"] + cfg["outcome"]["follow-up"] + cfg["outcome"]["failed"]
    expected_method_score = cfg["method"]["meeting"] + cfg["method"]["call"] + cfg["method"]["email"]
    expected_duration_score = (35 // 10 * cfg["duration_bonus_per_10_min"]) + (12 // 10 * cfg["duration_bonus_per_10_min"])
    ...
    mock_agg_result.one_or_none.return_value = MagicMock(
        ...
        last_consultation_date=mock_consultations_data[0]["consultation_date"] # Most recent
    )
...
async def test_calculate_engagement_score(mock_db_session):
    ...
    score = await insights_service._calculate_engagement_score(mock_db_session, mock_lead.id)
    ...
    expected_base = (
        ...
        (cfg["outcome"]["successful"] + cfg["outcome"]["follow-up"] + cfg["outcome"]["failed"]) + 
        (cfg["method"]["meeting"] + cfg["method"]["call"] + cfg["method"]["email"]) + 
        ((35 // 10 * cfg["duration_bonus_per_10_min"]) + (12 // 10 * cfg["duration_bonus_per_10_min"]))
    )
    expected_penalty = 0

    expected_score = int(max(0, min(expected_base - expected_penalty, cfg["max_score"])))
...
async def test_get_lead_insights_overall_score_calculation(mock_db_session, mocker):
    ...
    mocker.patch('app.services.insights_service._calculate_engagement_score', new_callable=AsyncMock, return_value=80)
    mocker.patch('app.services.insights_service._calculate_fit_score', return_value=60)
    mocker.patch('app.services.insights_service._calculate_urgency_score', return_value=50)

    # Note: mock_lead has officer_rating=4
    lead_insights = await insights_service.get_lead_insights(mock_db_session, mock_lead, mock_timeline)
    ...
    expected_raw_score = (
        ...
        (60 * weights["fit"]) + 
        (50 * weights["urgency"]) +
        (mock_lead.officer_rating * weights["officer_rating_multiplier"] * weights["officer_rating_weight"])
    )
    ...
    mock_db_session.refresh.assert_awaited_once_with(mock_lead, ["assignment_logs", "pipeline_stage"])