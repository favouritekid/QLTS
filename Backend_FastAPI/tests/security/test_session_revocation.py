...
async def test_targeted_revocation_flow(test_server, client, regular_user_in_db, test_redis_client):
    ...
    Skips gracefully if server is unavailable (e.g., in Docker/CI containers).
    """
    import aiohttp
    ...
    try:
        ...
        except (socketio.exceptions.ConnectionError, Exception) as conn_err:
            await session_a.close()
            await session_b.close()
            pytest.skip(f"Socket.IO server unavailable (requires real Uvicorn): {conn_err}")
