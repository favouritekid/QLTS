...
def test_create_access_token_default_expiry():
    ...
    try:
        ...
        assert required_keys.issubset(payload.keys()), f"Payload thiếu keys: {required_keys - payload.keys()}"

        # 2. Kiểm tra giá trị như mong đợi
        assert payload["sub"] == TEST_USERNAME, f"Expected sub='{TEST_USERNAME}', got '{payload['sub']}'"
        assert payload["type"] == "access", f"Expected type='access', got '{payload['type']}'"
        ...
        assert not unexpected_keys.intersection(payload.keys()), f"Payload chứa key không mong muốn: {unexpected_keys.intersection(payload.keys())}"
        ...
        assert abs(expected_expiry - actual_expiry) < timedelta(seconds=10), "Thời gian hết hạn không khớp (sai số > 10s)" # Tăng sai số lên 10s
...
def test_create_access_token_custom_expiry():
    ...
    token = security.create_access_token(data=TEST_DATA_PAYLOAD, expires_delta=custom_delta)
    log.debug(f"Generated Token: {token}")
    ...
    try:
        ...
        actual_expiry = datetime.fromtimestamp(payload["exp"], tz=timezone.utc)
        assert abs(expected_expiry - actual_expiry) < timedelta(seconds=10), "Thời gian hết hạn tùy chỉnh không khớp (sai số > 10s)"
...
def test_create_refresh_token_default_expiry():
    ...
    try:
        ...
        expected_expiry = datetime.now(timezone.utc) + timedelta(days=settings.REFRESH_TOKEN_EXPIRE_DAYS)
        actual_expiry = datetime.fromtimestamp(payload["exp"], tz=timezone.utc)
        assert abs(expected_expiry - actual_expiry) < timedelta(seconds=10), "Thời gian hết hạn mặc định không khớp (sai số > 10s)"
...
def test_create_refresh_token_custom_expiry():
    ...
    token = security.create_refresh_token(data=TEST_DATA_PAYLOAD, expires_delta=custom_delta)
    log.debug(f"Generated Token: {token}")
...
def test_create_and_verify_password_reset_token_success():
    ...
    assert verified_email == email, f"Expected verified email '{email}', got '{verified_email}'"
    ...
    try:
        ...
        assert payload.get("scope") == "password_reset", "Scope phải là 'password_reset'"
...
@patch('jose.jwt.decode', side_effect=JWTError("Simulated Token has expired")) # Mock lỗi decode
...
def test_verify_password_reset_token_wrong_scope():
    ...
    wrong_scope_token = jwt.encode(wrong_scope_payload, settings.JWT_SECRET_KEY, algorithm=settings.JWT_ALGORITHM)
...
def test_decode_token_for_invalidation_success():
    ...
    try:
        original_payload = jwt.decode(token, settings.JWT_SECRET_KEY, algorithms=[settings.JWT_ALGORITHM])
    ...
    assert abs(ttl - int(expected_ttl_seconds)) <= 10, f"TTL ({ttl}s) không nằm trong khoảng mong đợi (~{int(expected_ttl_seconds)}s +/- 10s)"
...
def test_decode_token_for_invalidation_invalid_token_string():
    ...
    jti, ttl = security.decode_token_for_invalidation(SecurityConstants.INVALID_TOKEN_STRING) # Dùng constant
    ...
    log.info("--- Finished: test_decode_token_for_invalidation_invalid_token_string ---")
...
def test_decode_token_for_invalidation_expired_token():
    ...
    expired_token = security.create_access_token(data=TEST_DATA_PAYLOAD, expires_delta=delta)

     # Decode gốc để lấy JTI mong đợi
    try:
        original_payload = jwt.decode(expired_token, settings.JWT_SECRET_KEY, algorithms=[settings.JWT_ALGORITHM], options={"verify_exp": False}) # Tắt kiểm tra hết hạn
        expected_jti = original_payload.get("jti")
    ...
    assert isinstance(ttl, int), "TTL trả về phải là integer"
    assert ttl <= 0, f"TTL cho token đã hết hạn phải <= 0, got {ttl}" # TTL còn lại phải là 0 hoặc âm

    log.info(f"Expired token correctly returned JTI ({jti}) and non-positive TTL ({ttl}).")
...
def test_decode_token_for_invalidation_no_exp():
    ...
    token_no_exp = jwt.encode(payload_no_exp, settings.JWT_SECRET_KEY, algorithm=settings.JWT_ALGORITHM)
