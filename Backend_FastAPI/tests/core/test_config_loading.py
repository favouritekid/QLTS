...
def test_simulate_load_from_env_file(reload_settings_and_clear_env):
    ...
    file_values = {
        'APP_ENV': 'test', 'SECRET_KEY': 'test_secret_from_file',
        'DATABASE_URL': 'postgresql+asyncpg://test_user:test_pass@localhost:5432/test_db_from_file',
        'JWT_SECRET_KEY': 'test_jwt_secret_from_file', 'MAIL_USERNAME': 'test_mail_user_from_file',
        'MAIL_PASSWORD': 'test_mail_pass_from_file', 'MAIL_FROM': 'test_mail_from@example.com',
        'MAIL_SERVER': 'smtp.test_from_file.com', 'ACCESS_TOKEN_EXPIRE_MINUTES': '60',
        'CELERY_BROKER_URL': 'redis://file_broker/9', 'REFRESH_TOKEN_EXPIRE_DAYS': '0.001',
        'MAIL_PORT': '2525',
    }
...
def test_override_simulated_env_file_with_os_env(reload_settings_and_clear_env):
    ...
    file_values = {
        'APP_ENV': 'test', 'SECRET_KEY': 'test_secret_from_file', 'DATABASE_URL': 'db_from_file',
        'JWT_SECRET_KEY': 'jwt_from_file', 'MAIL_USERNAME': 'user_from_file',
        'MAIL_PASSWORD': 'pass_from_file', 'MAIL_FROM': 'from@file.com',
        'MAIL_SERVER': 'server.file.com', 'ACCESS_TOKEN_EXPIRE_MINUTES': '60',
    }
    override_values = {
        'SECRET_KEY': 'secret_from_os_env', 'ACCESS_TOKEN_EXPIRE_MINUTES': '99',
    ...
    with patch.dict(os.environ, final_env, clear=True):
        with patch('app.config.os.path.exists', return_value=False):
            ...
            assert settings.MAIL_PORT == 587
            log.info("OS environment variables correctly override simulated file values.")
...
def test_required_variables_missing(reload_settings_and_clear_env):
    ...
    with patch.dict(os.environ, env_vars_to_set, clear=True):
         with patch('app.config.os.path.exists', return_value=False):
             ...
             log.info("ValidationError raised correctly during reload when required variables are missing.")
...
def test_calculated_values(mock_makedirs, mock_path_join, reload_settings_and_clear_env):
    ...
    with patch.dict(os.environ, env_vars, clear=True):
        with patch('app.config.os.path.exists', return_value=False):
            ...
            mock_makedirs.assert_called_with(settings.AVATAR_UPLOAD_FOLDER, exist_ok=True)
...
def test_type_coercion(reload_settings_and_clear_env):
    ...
    env_vars = {
        **get_minimal_required_env(),
        'ACCESS_TOKEN_EXPIRE_MINUTES': '35', 'REFRESH_TOKEN_EXPIRE_DAYS': '7.5',
        'MAIL_PORT': '1025', 'MAIL_STARTTLS': 'false', 'MAIL_SSL_TLS': 'True',
