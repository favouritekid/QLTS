...
@pytest.mark.asyncio
async def test_logout_success(client: AsyncClient, regular_user_in_db: dict, test_redis_client):
    """
    ...
    assert access_jti and access_ttl is not None and access_ttl > 0, "Could not decode access token JTI/TTL"

    refresh_jti_key = f"refresh_jti:{user_id}"
    refresh_jti_before_logout = await test_redis_client.get(refresh_jti_key)
    assert refresh_jti_before_logout is not None, "Refresh JTI should exist in Redis before logout"
    log.debug(f"Access JTI: {access_jti}, Refresh JTI before logout: {refresh_jti_before_logout}")

    # --- Bước 2: Gọi API logout ---
    response = await client.post(AuthURLs.LOGOUT, headers=headers) # Dùng constant URL

    # --- Assert Response ---
    assert response.status_code == 204, f"Logout Resp: Status {response.status_code}, Text: {response.text}"
    ...
    async with AsyncSessionLocal() as session:
        ...
        assert db_user.active_jti is None, "active_jti should be set to None in DB after logout"
    ...
    assert is_access_blacklisted == 1, f"Access JTI '{access_jti}' should be blacklisted in Redis"
    ...
    assert access_blacklist_ttl > 0 and abs(access_blacklist_ttl - access_ttl) <= 10, \
           f"Unexpected TTL for access token blacklist: {access_blacklist_ttl} (original TTL ~{access_ttl})"
    log.info(f"Redis check OK: Access JTI '{access_jti}' is blacklisted with TTL ~{access_blacklist_ttl}s.")

    stored_refresh_jti_after = await test_redis_client.get(refresh_jti_key)
    assert stored_refresh_jti_after is None, f"Redis key '{refresh_jti_key}' should be deleted after logout"
    ...
    assert is_refresh_blacklisted == 1, f"Refresh JTI '{refresh_jti_before_logout}' should be blacklisted in Redis"
    ...
    assert refresh_blacklist_ttl > 0 and abs(refresh_blacklist_ttl - expected_refresh_ttl) <= 10, \
           f"Unexpected TTL for refresh token blacklist: {refresh_blacklist_ttl}"
    log.info(f"Redis check OK: Refresh JTI '{refresh_jti_before_logout}' is blacklisted with TTL ~{refresh_blacklist_ttl}s.")
    ...
    profile_res = await client.get(ProfileURLs.PROFILE, headers=headers) # Dùng constant URL
    assert profile_res.status_code == 401, "Logged-out access token should result in 401"
    ...
    log.info("Using logged-out access token correctly failed with 401 and specific message.")
...
@pytest.mark.asyncio
async def test_logout_twice_with_same_token(client: AsyncClient, regular_user_in_db: dict, test_redis_client):
    ...
    assert response2.status_code == 401, "Second logout with same token should fail (401)"
