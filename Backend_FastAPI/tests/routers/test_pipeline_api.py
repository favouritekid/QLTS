...
from sqlalchemy import select, func# Cần để kiểm tra DB (mặc dù fixture đã seed)

# Import các thành phần app
from app.database import AsyncSessionLocal
from app import models # Cần để tạo dữ liệu mẫu
from app.services.pipeline_service import PIPELINE_STAGES_CACHE_KEY, PIPELINE_STATUSES_CACHE_KEY # Import cache keys
...
try:
    ...
    from ..fixtures.constants import (
        PipelineURLs,
        TestPipelineData,
        # Không cần các constants khác cho file này
    )
except ImportError:
...
async def seed_pipeline_data(setup_test_database):
    ...
    status1_2_data = {"id": "S1_2", "name": "Status 1.2", "color_code": "#00FF00", "stage_id": stage1_data["id"]} # Thêm status thứ 2 cho stage A
    ...
    seeded_statuses = sorted([status1_1_data, status1_2_data, status2_1_data], key=lambda x: x["id"])
    return {
...
async def test_get_pipeline_all_success_cache_miss(
    ...
    assert len(data["stages"]) == len(expected_stages), f"Expected {len(expected_stages)} stages, got {len(data['stages'])}"
    assert len(data["statuses"]) == len(expected_statuses), f"Expected {len(expected_statuses)} statuses, got {len(data['statuses'])}"
    ...
    async with AsyncSessionLocal() as session:
        ...
        status_count = await session.scalar(select(func.count(models.ConsultationStatus.id)))
    ...
    assert cached_stages_str is not None, f"Cache key '{PIPELINE_STAGES_CACHE_KEY}' not found after GET"
    assert cached_statuses_str is not None, f"Cache key '{PIPELINE_STATUSES_CACHE_KEY}' not found after GET"
    ...
    try:
        ...
        assert cached_statuses_sorted == expected_statuses, "Cached statuses content mismatch"
        ...
        assert stages_ttl > 0 and stages_ttl <= expected_ttl, f"Unexpected stages cache TTL: {stages_ttl}"
        assert statuses_ttl > 0 and statuses_ttl <= expected_ttl, f"Unexpected statuses cache TTL: {statuses_ttl}"
...
async def test_get_pipeline_all_success_cache_hit(
    ...
    fake_statuses_data = [{"id": "FAKE_T", "name": "Fake Status CACHED", "color_code": "#111111", "stage_id": "FAKE_S"}]

    await test_redis_client.set(PIPELINE_STAGES_CACHE_KEY, json.dumps(fake_stages_data))
    await test_redis_client.set(PIPELINE_STATUSES_CACHE_KEY, json.dumps(fake_statuses_data))
    ...
    assert data["stages"] == fake_stages_data, "Response stages should match FAKE cached data"
    assert data["statuses"] == fake_statuses_data, "Response statuses should match FAKE cached data"
    ...
    async with AsyncSessionLocal() as session:
        stage_count = await session.scalar(select(func.count(models.PipelineStage.id)))
        status_count = await session.scalar(select(func.count(models.ConsultationStatus.id)))
        assert stage_count == len(seed_pipeline_data["stages"]), "DB stage count should remain unchanged"
        assert status_count == len(seed_pipeline_data["statuses"]), "DB status count should remain unchanged"
