...
import logging
from datetime import datetime, timedelta, timezone  # Cần datetime và timezone

import pytest
import pytest_asyncio  # Import nếu chưa có
from httpx import AsyncClient, Response  # Import Response để kiểm tra kiểu
from jose import JWTError, jwt  # Cần để decode token mới
...
from app.security import create_refresh_token  # Cần create_refresh_token
...
async def get_tokens(client: AsyncClient, username: str, password: str) -> dict:
    ...
    if login_res.status_code != 200:
        pytest.fail(
            f"Helper get_tokens failed: Login returned {login_res.status_code} - {login_res.text}"
        )
...
async def test_refresh_success(
    ...
    initial_refresh_jti, initial_refresh_ttl = decode_token_for_invalidation(
    ...
    assert (
        initial_refresh_ttl is not None and initial_refresh_ttl > 0
    ), "Could not decode initial refresh TTL"
    log.debug(
        f"Initial Tokens - Access JTI: {initial_access_jti}, Refresh JTI: {initial_refresh_jti}"
    )

    # --- Bước 2: Gọi API refresh ---
    log.info(f"Calling refresh endpoint: {AuthURLs.REFRESH}")
    response = await client.post(
        AuthURLs.REFRESH, json={"refresh_token": initial_refresh_token}
    )
    ...
    log.info(
        "Refresh successful (200). Response structure and token rotation verified."
    )
    ...
    try:
        ...
        log.debug(
            f"New Tokens - Access JTI: {new_access_jti}, Refresh JTI: {new_refresh_jti}"
        )
    ...
    async with AsyncSessionLocal() as session:
        ...
        assert (
            db_user.active_jti == new_access_jti
        ), "active_jti in DB should match NEW access token JTI"
    log.info(
        f"DB state verified: active_jti matches NEW access JTI '{new_access_jti}'."
    )

    # 3.2: Assert Redis State
    #   a) Refresh Token JTI CŨ bị blacklist
    old_refresh_blacklist_key = f"blacklist:{initial_refresh_jti}"
    is_old_refresh_blacklisted = await test_redis_client.exists(
        old_refresh_blacklist_key
    )
    assert (
        is_old_refresh_blacklisted == 1
    ), f"Old refresh JTI '{initial_refresh_jti}' should be blacklisted"
    # Blacklist này thường có TTL ngắn (ví dụ 300s)
    old_blacklist_ttl = await test_redis_client.ttl(old_refresh_blacklist_key)
    assert (
        old_blacklist_ttl > 0 and old_blacklist_ttl <= 300
    ), f"Unexpected TTL for old refresh JTI blacklist: {old_blacklist_ttl}"
    log.info(
        f"Redis check OK: Old refresh JTI '{initial_refresh_jti}' blacklisted with TTL ~{old_blacklist_ttl}s."
    )
    ...
    assert (
        stored_new_refresh_jti == new_refresh_jti
    ), "Redis should store the NEW refresh JTI"
    ...
    assert (
        new_refresh_key_ttl_actual > 0
        and abs(new_refresh_key_ttl_actual - new_refresh_ttl_expected) <= 10
    ), f"Unexpected TTL for new refresh JTI key: {new_refresh_key_ttl_actual} (expected ~{new_refresh_ttl_expected})"
    log.info(
        f"Redis check OK: '{refresh_jti_key}' stores NEW refresh JTI '{new_refresh_jti}' with TTL ~{new_refresh_key_ttl_actual}s."
    )

    # --- Bước 4: Kiểm tra dùng Access Token MỚI -> Thành công ---
    log.info("Attempting to use NEW access token...")
    new_headers = {"Authorization": f"Bearer {new_access_token}"}
    profile_res_new = await client.get(
        ProfileURLs.PROFILE, headers=new_headers
    )  # Dùng constant URL
    assert (
        profile_res_new.status_code == 200
    ), f"NEW access token should be valid, but got {profile_res_new.status_code}"
    ...
    assert (
        profile_res_old.status_code == 401
    ), "OLD access token should be invalid after refresh"
    ...
    assert (
        reuse_response.status_code == 401
    ), "Reusing OLD refresh token should fail (401)"
...
async def test_refresh_invalid_token_string(client: AsyncClient):
    ...
    response = await client.post(
        AuthURLs.REFRESH, json={"refresh_token": SecurityConstants.INVALID_TOKEN_STRING}
    )  # Dùng constant
    ...
    assert (
        error_data["detail"] == "Invalid or expired refresh token"
    )  # Message lỗi cụ thể
    log.info(
        "Refresh with invalid token string correctly failed (401) with specific message."
    )
...
async def test_refresh_expired_token(client: AsyncClient, regular_user_in_db: dict):
    ...
    response = await client.post(
        AuthURLs.REFRESH, json={"refresh_token": expired_token}
    )
...
async def test_refresh_blacklisted_token(
    ...
    assert (
        is_blacklisted == 1
    ), f"Refresh JTI '{refresh_jti}' should be blacklisted after logout"

    # --- Thử refresh với token đã blacklist ---
    log.info("Attempting refresh with blacklisted token...")
    response = await client.post(
        AuthURLs.REFRESH, json={"refresh_token": refresh_token_to_blacklist}
    )
    ...
    assert (
        error_data["detail"] == "Invalid or expired refresh token"
    )  # Lỗi do blacklist check
    log.info(
        "Refresh with blacklisted token correctly failed (401) with specific message."
    )
...
@pytest.mark.asyncio
async def test_refresh_concurrent_requests(
    client: AsyncClient, regular_user_in_db: dict, test_redis_client
):
    """
    ...
    log.info(
        f"Using refresh token with JTI: {concurrent_refresh_jti} for concurrent test"
    )
    ...
    for i in range(num_concurrent_requests):
        tasks.append(
            client.post(
                AuthURLs.REFRESH, json={"refresh_token": concurrent_refresh_token}
            )
        )
    ...
    results: List[Response | Exception] = await asyncio.gather(
            ...
            if result.status_code == 200:
                ...
                try:
                    ...
                    if new_at:
                        ...
                        if jti_at:
                            new_access_jti_from_success = jti_at
                    if new_rt:
                        jti_rt, _ = decode_token_for_invalidation(new_rt)
                        if jti_rt:
                            new_refresh_jti_from_success = jti_rt
                except Exception as e:
                    log.warning(
                        f"Could not parse new JTIs from successful response {i+1}: {e}"
                    )
            elif result.status_code in [
                400,
                401,
            ]:  # Lỗi dự kiến do race condition/blacklist
                fail_401_400_count += 1
                # Kiểm tra message lỗi cụ thể
                try:
                    error_detail = result.json().get("detail", "")
                    assert (
                        "Invalid or expired refresh token" in error_detail
                        or "Could not validate credentials" in error_detail
                    )  # Có thể là lỗi JTI mismatch từ service
                except Exception:
                    log.warning(
                        f"Could not parse detail from failed response {i+1} ({result.status_code})"
                    )
            else:
                log.error(
                    f"Request {i+1} unexpected status code: {result.status_code} - {result.text}"
                )
    ...
    assert (
        success_count == 1
    ), f"Expected exactly 1 successful request, but got {success_count}"
    assert (
        fail_401_400_count == num_concurrent_requests - 1
    ), f"Expected {num_concurrent_requests - 1} failed requests (400/401), but got {fail_401_400_count}"
    assert (
        len(other_failures) == 0
    ), f"Got unexpected failures or exceptions: {other_failures}"
    log.info(
        f"Concurrent execution result OK: {success_count} success, {fail_401_400_count} failures (400/401)."
    )

    # --- Assert Side Effects Cuối Cùng ---
    # 1. Token gốc phải bị blacklist
    is_original_blacklisted = await test_redis_client.exists(
        f"blacklist:{concurrent_refresh_jti}"
    )
    assert (
        is_original_blacklisted == 1
    ), f"Original refresh token JTI '{concurrent_refresh_jti}' should be blacklisted"
    log.info(
        f"Redis check OK: Original refresh JTI '{concurrent_refresh_jti}' is blacklisted."
    )

    # 2. Redis key và DB phải chứa JTI mới (từ request thành công duy nhất)
    assert (
        new_access_jti_from_success is not None
    ), "Could not extract new access JTI from successful response"
    assert (
        new_refresh_jti_from_success is not None
    ), "Could not extract new refresh JTI from successful response"

    stored_final_refresh_jti = await test_redis_client.get(f"refresh_jti:{user_id}")
    assert (
        stored_final_refresh_jti == new_refresh_jti_from_success
    ), "Redis should store the JTI from the single successful request"
    log.info(
        f"Redis check OK: refresh_jti:{user_id} stores the final correct JTI '{new_refresh_jti_from_success}'."
    )
    ...
    async with AsyncSessionLocal() as session:
        ...
        assert (
            final_db_user.active_jti == new_access_jti_from_success
        ), "DB active_jti should match the JTI from the single successful request"
    log.info(
        f"DB check OK: active_jti stores the final correct JTI '{new_access_jti_from_success}'."
    )
