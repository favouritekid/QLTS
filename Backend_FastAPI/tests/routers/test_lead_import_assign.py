...
from ..fixtures.constants import (
    ...
    TestOrgData,
    TestPipelineData,
    # TestLeadData # Không cần trực tiếp vì dùng data custom
)
# Import helper tạo file mock
from ..conftest import create_mock_lead_file # ✅ Đường dẫn import đúng (từ thư mục cha)
...
VALID_LEAD_DATA_FOR_FILE = [
    {"full_name": "Import Lead 1", "email": "import1@example.com", "phone": "1110001", "source": "file_import", "unit_id": FILE_DATA_UNIT_ID},
    {"full_name": "Import Lead 2", "email": "import2@example.com", "phone": "1110002", "source": "file_import", "unit_id": FILE_DATA_UNIT_ID, "major_id": FILE_DATA_MAJOR_ID},
]

INVALID_LEAD_DATA_FOR_FILE = [
    {"full_name": "Import Lead 3 Valid", "email": "import3@example.com", "phone": "1110003", "source": "file_import", "unit_id": FILE_DATA_UNIT_ID},
    {"full_name": "Missing Email", "phone": "1110004", "source": "file_import", "unit_id": FILE_DATA_UNIT_ID}, # Thiếu email
    {"full_name": "Bad Email Format", "email": "bad-email", "phone": "1110005", "source": "file_import", "unit_id": FILE_DATA_UNIT_ID}, # Sai email
    {"full_name": "Duplicate Email In File", "email": "import3@example.com", "phone": "1110006", "source": "file_import", "unit_id": FILE_DATA_UNIT_ID}, # Trùng email dòng 1
]
# ------------------------------------
...
async def test_import_and_bulk_assign_success(
    ...
    file_data = [
        {"full_name": "Import Assign 1", "email": "imp_assign1@example.com", "phone": "2220001", "source": "import_test", "unit_id": unit_id, "major_id": major_id},
        {"full_name": "Import Assign 2", "email": "imp_assign2@example.com", "phone": "2220002", "source": "import_test", "unit_id": unit_id}, # Ko major
        {"full_name": "Import Assign 3", "email": "imp_assign3@example.com", "phone": "2220003", "source": "import_test", "unit_id": unit_id, "major_id": major_id},
    ]
    ...
    async with AsyncSessionLocal() as session:
        leads_after_import = (await session.execute(
    ...
    assert assign_response.status_code == 202, f"Bulk assign call failed: {assign_response.text}"
    ...
    unassigned_count = 0
    async with AsyncSessionLocal() as session:
        leads_after_assign = (await session.execute(
            select(models.Lead).where(models.Lead.id.in_(created_lead_ids))
        )).scalars().all()
    ...
    log.info(f"Assignment verification: Assigned={assigned_count}, Unassigned={unassigned_count}")
    assert assigned_count > 0, "Expected at least one lead to be assigned"
    assert unassigned_count == (len(file_data) - assigned_count), "Unassigned count mismatch"
...
async def test_import_with_errors(
    ...
    for row in file_data_with_errors:
        if 'unit_id' in row: row['unit_id'] = unit_id
    ...
    # --- Assert Import Response ---
    assert import_response.status_code == 200, f"Import with errors failed: {import_response.text}"
    import_result = import_response.json()
    ...
    assert "email" in errors[0]["error_message"] and "valid email address" in errors[0]["error_message"]
    assert "input_value='nan'" in errors[0]["error_message"] # Kiểm tra lỗi cụ thể 'nan'

    # Lỗi 2: Dòng 4 (Bad Email Format)
    assert errors[1]["row_number"] == 4
    assert "email" in errors[1]["error_message"] and "valid email address" in errors[1]["error_message"]
    assert "input_value='bad-email'" in errors[1]["error_message"] # Kiểm tra lỗi 'bad-email'

    # Lỗi 3: Dòng 5 (Duplicate Email)
    assert errors[2]["row_number"] == 5
    assert "Email" in errors[2]["error_message"] and "already exists" in errors[2]["error_message"]
    
    log.info("Import with errors handled correctly. Response verified.")

    # --- Assert DB State ---
    async with AsyncSessionLocal() as session:
        created_lead = (await session.execute(
            select(models.Lead).where(models.Lead.email == "import3@example.com") # Email hợp lệ duy nhất
        )).scalar_one_or_none()
        assert created_lead is not None
        assert created_lead.id == import_result["created_lead_ids"][0]

        total_leads_in_db = (await session.execute(select(func.count(models.Lead.id)))).scalar_one()
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_import_invalid_file_format(client: AsyncClient, admin_token_headers: dict):
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_bulk_assign_unauthorized(client: AsyncClient, officer_token_headers: dict):
...
async def test_bulk_assign_empty_list(client: AsyncClient, admin_token_headers: dict):
    ...
    for err in error_data["errors"]:
        if err.get("field") == "body -> lead_ids" and "List should have at least 1 item" in err.get("message", ""):
            found_error = True
            break
    assert found_error
