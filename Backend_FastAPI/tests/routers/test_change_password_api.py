...
from app.security import verify_password, decode_token_for_invalidation # Cần verify_password và decode
...
try:
    ...
    from ..fixtures.constants import (
        AuthURLs,
        TestUsers,
        ProfileURLs, # Cần ProfileURLs để test token cũ
...
@pytest.mark.asyncio
async def test_change_password_success(client: AsyncClient, regular_user_in_db: dict, test_redis_client):
    """
    ...
    assert old_refresh_jti is not None, "Refresh JTI should exist in Redis before password change"
    ...
    response = await client.post(AuthURLs.CHANGE_PASSWORD, json=payload, headers=old_headers) # Dùng constant URL

    # --- Assert Response ---
    assert response.status_code == 204, f"Change Password Resp: Status {response.status_code}, Text: {response.text}" # No Content on success
    ...
    async with AsyncSessionLocal() as session:
        ...
        assert db_user is not None, f"User {user_id} not found in DB after password change"
    ...
    assert is_user_blacklisted == 1, f"User ID {user_id} should be in user_blacklist in Redis"
    ...
    assert blacklist_ttl > 0 and abs(blacklist_ttl - expected_max_ttl) <= 10, f"Unexpected TTL for user_blacklist: {blacklist_ttl}"
    ...
    assert stored_refresh_jti is None, f"Redis key '{refresh_jti_key}' should be deleted after password change"
    ...
    profile_res = await client.get(ProfileURLs.PROFILE, headers=old_headers) # Dùng constant URL
    assert profile_res.status_code == 401, "Old access token should be invalid (due to user blacklist)"
...
@pytest.mark.asyncio
async def test_change_password_incorrect_old_password(client: AsyncClient, regular_user_token_headers: dict):
    ...
    response = await client.post(AuthURLs.CHANGE_PASSWORD, json=payload, headers=regular_user_token_headers)

    # --- Assert Error Response ---
    assert response.status_code == 400, f"Resp: {response.text}" # Bad Request
    error_data = response.json()
    assert "detail" in error_data
    assert error_data["detail"] == "Incorrect old password" # Message lỗi cụ thể từ service
...
@pytest.mark.asyncio
async def test_change_password_weak_new_password(client: AsyncClient, regular_user_token_headers: dict, regular_user_in_db: dict):
    ...
    for error in error_data["errors"]:
        ...
        # --- KẾT THÚC SỬA ---
            ...
            assert ("at least 8 characters" in error.get("message", "") or
    ...
    assert found_error, "Validation error for weak 'new_password' field not found in errors list"
...
async def test_change_password_unauthenticated(client: AsyncClient):
    ...
    payload = {
        "old_password": "any_old",
        "new_password": "NewStrongPassword!123"
    }
    response = await client.post(AuthURLs.CHANGE_PASSWORD, json=payload) # Không có headers

    # --- Assert Error Response ---
    assert response.status_code == 401, f"Resp: {response.text}" # Unauthorized
    error_data = response.json()
    assert "detail" in error_data
    assert error_data["detail"] == "Not authenticated" # Message mặc định của FastAPI/Starlette
