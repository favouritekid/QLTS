...
# Import constants
try:
    from ..fixtures.constants import AdminURLs, TestUsers, NON_EXISTENT_ID
except ImportError:
...
async def test_admin_casbin_policy_crud_flow(
    ...
    setup_test_database # Cần DB sạch
):
    """
    ...
    # Assert DB
    db_rule_post = await get_casbin_rule_from_db('p', policy_schema.subject, policy_schema.object, policy_schema.action)
    ...
    # Assert DB
    db_rule_del = await get_casbin_rule_from_db('p', policy_schema.subject, policy_schema.object, policy_schema.action)
    ...
    # Assert Response
    assert response_del_404.status_code == 404, f"DELETE 404 Resp: {response_del_404.text}"
...
async def test_admin_casbin_role_crud_flow(
    ...
    # Assert DB
    db_rule_post = await get_casbin_rule_from_db('g', f"user:{user_id}", role_schema.role)
    assert db_rule_post is not None, "Role assignment (g rule) not found in DB after creation"
    ...
    # Assert Response
    assert response_dupe.status_code == 409, f"POST Dupe Role Resp: {response_dupe.text}"
    ...
    # Assert DB
    db_rule_del = await get_casbin_rule_from_db('g', f"user:{user_id}", role_schema.role)
    assert db_rule_del is None, "Role assignment (g rule) still found in DB after deletion"
    ...
    # Assert Response
    assert response_del_404.status_code == 404, f"DELETE Role 404 Resp: {response_del_404.text}"
