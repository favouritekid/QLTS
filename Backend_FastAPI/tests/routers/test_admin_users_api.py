...
@pytest.mark.asyncio
async def test_admin_get_users_list(client: AsyncClient, admin_token_headers: dict, admin_user_in_db: dict, regular_user_in_db: dict):
    ...
    assert data["total_count"] >= 2, f"Expected at least 2 users (admin, regular), got total_count={data['total_count']}"
    ...
    assert TestUsers.ADMIN["username"] in usernames_in_response, "Admin user not found in response list"
    assert TestUsers.REGULAR["username"] in usernames_in_response, "Regular user not found in response list"
    log.info("Get users list successful. Response structure and content verified.")

@pytest.mark.asyncio
async def test_admin_get_users_list_unauthorized(client: AsyncClient, regular_user_token_headers: dict):
    ...
    assert response.status_code == 403, f"Resp: {response.text}" # Forbidden (Casbin check)
...
@pytest.mark.asyncio
async def test_admin_get_users_list_pagination(client: AsyncClient, admin_token_headers: dict, setup_test_database): # Thêm setup_test_database
    ...
    for i in range(num_extra_users):
         ...
         create_response = await client.post(AdminURLs.USERS, data={
         ...
         assert create_response.status_code == 201, f"Failed to create pageuser{i}: {create_response.text}"
    ...
    response1 = await client.get(f"{AdminURLs.USERS}?page=1&page_size={page_size}", headers=admin_token_headers)
    ...
    assert data1["total_count"] == expected_total_count, f"Total count mismatch, expected {expected_total_count}"
    ...
    response2 = await client.get(f"{AdminURLs.USERS}?page=2&page_size={page_size}", headers=admin_token_headers)
    ...
    assert data2["total_count"] == expected_total_count, f"Total count mismatch on page 2"
    page2_ids = {user["id"] for user in data2["users"]}
    assert not page1_ids.intersection(page2_ids), "Page 1 and Page 2 should have different users"
    log.info(f"Page 2 OK (Size: {len(data2['users'])}, Total: {data2['total_count']}). IDs are different from Page 1.")

    # --- Test Trang Cuối ---
    # Trang cuối là trang 4 (16 users / 5 per page = 3.2 -> 4 pages)
    last_page = (expected_total_count + page_size - 1) // page_size
    remaining_users = expected_total_count % page_size if expected_total_count % page_size != 0 else page_size
    log.info(f"Requesting Last Page ({last_page})...")
    response_last = await client.get(f"{AdminURLs.USERS}?page={last_page}&page_size={page_size}", headers=admin_token_headers)
    assert response_last.status_code == 200, f"Last Page Resp: {response_last.text}"
    data_last = response_last.json()
    assert len(data_last["users"]) == remaining_users, f"Last page should have {remaining_users} users"
    assert data_last["total_count"] == expected_total_count
    log.info(f"Last Page ({last_page}) OK (Size: {len(data_last['users'])}, Total: {data_last['total_count']}).")
...
@pytest.mark.asyncio
async def test_admin_create_user_success(client: AsyncClient, admin_token_headers: dict, setup_test_database): # Thêm setup_test_database
    ...
    response = await client.post(AdminURLs.USERS, data=new_user_data, headers=admin_token_headers)
    ...
    async with AsyncSessionLocal() as session:
        ...
        assert verify_password(new_user_data["password"], db_user.password_hash), "Password hash in DB is incorrect"
    log.info("Database state verified successfully.")

@pytest.mark.asyncio
async def test_admin_create_user_duplicate_username(client: AsyncClient, admin_token_headers: dict, regular_user_in_db: dict):
...
@pytest.mark.asyncio
async def test_admin_create_user_duplicate_email(client: AsyncClient, admin_token_headers: dict, regular_user_in_db: dict):
...
@pytest.mark.asyncio
async def test_admin_create_user_weak_password(client: AsyncClient, admin_token_headers: dict):
    ...
    for error in error_data["errors"]:
        if error.get("field") == "password":
            ...
            assert ("String should have at least 8 characters" in error.get("message", "") or
                    "Password must contain" in error.get("message", "")), \
...
@pytest.mark.asyncio
async def test_admin_get_user_detail_success(client: AsyncClient, admin_token_headers: dict, regular_user_in_db: dict):
...
@pytest.mark.asyncio
async def test_admin_get_user_detail_not_found(client: AsyncClient, admin_token_headers: dict):
...
@pytest.mark.asyncio
async def test_admin_update_user_success(client: AsyncClient, admin_token_headers: dict, regular_user_in_db: dict):
    ...
    response = await client.put(update_url, data=update_data, headers=admin_token_headers)
    ...
    async with AsyncSessionLocal() as session:
        ...
        assert db_user.max_capacity == int(update_data["max_capacity"]) # DB lưu dạng int
        assert db_user.skills == ["skillA", "skillB", "skillC"] # DB lưu dạng list/JSON
    log.info("Database state verified after update.")

@pytest.mark.asyncio
async def test_admin_update_user_duplicate_email(client: AsyncClient, admin_token_headers: dict, admin_user_in_db: dict, regular_user_in_db: dict):
    ...
    update_data = {
        "email": TestUsers.ADMIN["email"] # Email của admin đã tồn tại
    }
    response = await client.put(update_url, data=update_data, headers=admin_token_headers)
    ...
    log.info("Update with duplicate email correctly blocked (409) with specific message.")

@pytest.mark.asyncio
async def test_admin_update_user_not_found(client: AsyncClient, admin_token_headers: dict):
...
@pytest.mark.asyncio
async def test_admin_set_password_success(client: AsyncClient, admin_token_headers: dict, regular_user_in_db: dict):
    ...
    response = await client.post(set_password_url, json=payload, headers=admin_token_headers)

    # --- Assert Response ---
    assert response.status_code == 204, f"Set Password Resp: Status {response.status_code}" # No Content
...
@pytest.mark.asyncio
async def test_admin_set_password_user_not_found(client: AsyncClient, admin_token_headers: dict):
...
@pytest.mark.asyncio
async def test_admin_bulk_delete_success(client: AsyncClient, admin_token_headers: dict, regular_user_in_db: dict, admin_user_in_db: dict):
    ...
    user_to_delete_resp = await client.post(AdminURLs.USERS, data={
    ...
    ids_to_process = [regular_user_in_db["id"], user_to_delete_id, admin_user_in_db["id"]]
    payload = {"action": "delete", "user_ids": ids_to_process}
    response = await client.post(AdminURLs.BULK_ACTION, json=payload, headers=admin_token_headers)
    ...
    assert resp_data["message"] == "Successfully deleted 2 users.", f"Unexpected success message: {resp_data['message']}"
...
@pytest.mark.asyncio
async def test_admin_bulk_change_status_success(client: AsyncClient, admin_token_headers: dict, regular_user_in_db: dict):
    ...
    assert resp_data["message"] == f"Successfully updated status to '{new_status}' for 1 users."
    ...
    async with AsyncSessionLocal() as session:
        db_user = await session.get(User, user_id)
        assert db_user is not None
        assert db_user.status == new_status, f"User status should be '{new_status}', but got '{db_user.status}'"
    log.info("Database state verified: User status updated.")

@pytest.mark.asyncio
async def test_admin_bulk_action_change_status_missing_status(client: AsyncClient, admin_token_headers: dict):
    ...
    assert found_error, "Missing status error message not found in 422 response errors list"
    log.info("Missing status for change_status action correctly blocked (422).")

# === DELETE /api/admin/users/{user_id} ===

@pytest.mark.asyncio
async def test_admin_delete_user_success(client: AsyncClient, admin_token_headers: dict, regular_user_in_db: dict):
    ...
    # --- Assert Response ---
    assert response.status_code == 204, f"Delete Resp: Status {response.status_code}" # No Content

    # --- Assert DB State ---
    async with AsyncSessionLocal() as session:
        db_user = await session.get(User, user_id_to_delete)
        assert db_user is None, f"User ID {user_id_to_delete} should not exist in DB after delete"
    log.info("User deleted successfully. DB state verified.")

@pytest.mark.asyncio
async def test_admin_delete_self_fail(client: AsyncClient, admin_token_headers: dict, admin_user_in_db: dict):
    ...
    # --- Assert Error Response ---
    assert response.status_code == 403, f"Resp: {response.text}" # Forbidden (Logic trong service)
    error_data = response.json()
    assert "detail" in error_data
...
@pytest.mark.asyncio
async def test_admin_delete_user_not_found(client: AsyncClient, admin_token_headers: dict):
...
async def test_admin_update_user_avatar_file_too_large(
    ...
    assert response.status_code == status.HTTP_413_CONTENT_TOO_LARGE, f"Resp: {response.text}"
    error_data = response.json()
    assert "detail" in error_data
    assert error_data["detail"] == error_detail, "Message lỗi 413 không chính xác"
    log.info("Update user with large avatar correctly failed (413) with specific message.")
    ...
    async with AsyncSessionLocal() as session:
        ...
        assert db_user.full_name == regular_user_in_db.get("full_name") # Thường là None hoặc giá trị gốc
