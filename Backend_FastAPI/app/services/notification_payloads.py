...
class EventPayload:
    ...
    @staticmethod
    def for_application_survey_due(
        profile,
        *,
        lead_id: int,
        full_name: str,
        ...
        """Build APPLICATION_SURVEY_DUE payload for ZNS template 426903.

        Caller (``admission_survey_task``) resolves ``full_name`` and
        ``program_name`` via eager-load of ``profile.lead`` +
        ``offering_admission_config.offering.program`` and passes primitives —
        this builder stays pure per module contract. ``tracking_id`` is
        generated by the caller (UUID) and stored on
        ``admission_profile.survey_tracking_id`` in the same transaction so
        a later ``user_feedback`` webhook can map the Zalo-echoed value back
        to the originating profile.

        All four template slots are capped at 30 characters per the Zalo
        template spec (checked on the 2026-04-20 template preview); we
        silently truncate because applicant names or program names that
        exceed 30 chars are common and blocking the send would produce
        zero notifications instead of a slightly-cut display name.
        """
        from app.utils.datetime_helpers import format_vn_date
        from app.utils.id_helpers import format_profile_code

        clean_name = (full_name or "Unknown")[:30]
        clean_program = (program_name or "N/A")[:30]
        ...
        return {
            ...
            "profile_code": profile_code,
            "submitted_date_vn": format_vn_date(submitted_at),
