...
async def verify_intake_api_key(
    ...
    if not configured:
        # ⚠️ Thông điệp công khai KHÔNG nêu tên biến môi trường: nó nói cho người
        # lạ biết chính xác cần dò cái gì, và xác nhận endpoint có tồn tại. Chi
        # tiết đi vào log nội bộ, caller chỉ thấy 503 chung.
        log.warning("public intake: api key not configured")
