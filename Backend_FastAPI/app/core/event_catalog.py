...
_FINANCE_USER_EVENTS: tuple = (
    ...
    # Đổi ngành: reprice xong → HK1 fee chờ kế toán xác nhận (báo kế toán+admin).
    EventDefinition(
        event=SystemEvents.MAJOR_CHANGE_AWAITING_CONFIRMATION,
        category="finance",
        display_name="Đổi ngành — chờ kế toán xác nhận",
        description="Khi hồ sơ đổi ngành được định giá lại và chờ kế toán xác nhận học phí",
        variables=(
            _var("fee_id", "integer", "ID phí"),
            _var("invoice_id", "integer", "ID hóa đơn"),
            _var("admission_profile_id", "integer", "ID hồ sơ"),
            _var("lead_id", "integer", "ID lead", False),
            _var("unit_id", "integer", "ID đơn vị", False),
            ...
            _var("fee_version", "integer", "Phiên bản phí (dedup theo chu kỳ)", False),
        ),
        default_resolver="specific_users",
        allowed_resolvers=_FINANCE_RESOLVERS,
        default_channels=("browser", "email"),
        priority=50,
        # F11: gồm version → mỗi chu kỳ reprice có key riêng (call site truyền
        # dedupe_key versioned tường minh; template versioned làm fallback phòng thủ).
        dedup_key_template="fee:${fee_id}:v${fee_version}:major_change_awaiting",
        link_strategy="/finance/fees/${fee_id}",
        privacy="sensitive",
    ),
    # Đổi ngành: kế toán đã xác nhận → báo officer phụ trách tiếp tục (xuất giấy…).
    EventDefinition(
        event=SystemEvents.MAJOR_CHANGE_CONFIRMED,
        category="finance",
        display_name="Đổi ngành — kế toán đã xác nhận",
        description="Khi kế toán xác nhận học phí đổi ngành, officer có thể tiếp tục",
        variables=(
            _var("fee_id", "integer", "ID phí"),
            _var("invoice_id", "integer", "ID hóa đơn", False),
            _var("admission_profile_id", "integer", "ID hồ sơ"),
            _var("lead_id", "integer", "ID lead", False),
            _var("unit_id", "integer", "ID đơn vị", False),
            _var("new_major_name", "string", "Ngành mới", False),
            _var("profile_code", "string", "Mã hồ sơ pseudo (HS-{id})", False),
            _var("actor_id", "integer", "ID kế toán xác nhận", False),
            _var("fee_version", "integer", "Phiên bản phí (dedup theo chu kỳ)", False),
        ),
        default_resolver="specific_users",
        allowed_resolvers=_FINANCE_RESOLVERS,
        default_channels=("browser",),
        priority=55,
        # F11: gồm version (fallback phòng thủ; call site truyền key versioned).
        dedup_key_template="fee:${fee_id}:v${fee_version}:major_change_confirmed",
