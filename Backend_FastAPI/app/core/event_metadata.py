...
EVENT_METADATA_REGISTRY: Dict[SystemEvents, EventMetadata] = {
    ...
    SystemEvents.LEAD_CREATED: EventMetadata(
        ...
        variables=[
            ...
            EventVariable("lead_code", "string", "Mã lead pseudo (LEAD-{id})"),
            EventVariable("major_name", "string", "Tên ngành đăng ký (tối đa 30 ký tự)", required=False),
            EventVariable("created_date_vn", "string", "Ngày tạo VN (dd/mm/yyyy)", required=False),
        ],
    ...
    SystemEvents.CONSULTATION_REMINDER: EventMetadata(
        ...
        variables=[
            ...
            EventVariable("scheduled_time_vn", "string", "Thời gian hẹn VN (dd/mm/yyyy HH:MM)"),
            EventVariable("booking_code", "string", "Mã booking (CONS-{id})"),
            EventVariable("lead_code", "string", "Mã lead pseudo (LEAD-{id})"),
            EventVariable("major_name", "string", "Tên ngành (tối đa 30 ký tự)"),
        ],
    ...
    SystemEvents.INVOICE_ISSUED: EventMetadata(
        ...
        variables=[
            ...
            EventVariable("invoice_number", "string", "Số hóa đơn"),
            EventVariable("fee_id", "integer", "ID phí"),
            EventVariable("amount", "string", "Số tiền (raw Decimal string)"),
            EventVariable("due_date", "datetime", "Hạn thanh toán (ISO)"),
            EventVariable("admission_profile_id", "integer", "ID hồ sơ"),
            EventVariable("lead_id", "integer", "ID lead"),
            EventVariable("unit_id", "integer", "ID đơn vị"),
            EventVariable("user_id", "integer", "ID officer (recipient)"),
            EventVariable("profile_code", "string", "Mã hồ sơ pseudo (HS-{id})"),
            EventVariable("lead_full_name", "string", "Họ tên lead (tối đa 30 ký tự)", required=False),
            EventVariable("major_name", "string", "Tên ngành (tối đa 30 ký tự)", required=False),
            EventVariable("degree_level", "string", "Trình độ (Cao đẳng / Đại học)", required=False),
            EventVariable("amount_vnd", "string", "Số tiền VND dạng integer (không decimal)"),
            EventVariable("due_date_vn", "string", "Hạn thanh toán VN (dd/mm/yyyy)", required=False),
            EventVariable("bank_transfer_note", "string", "Nội dung chuyển khoản (ASCII, ≤90)", required=False),
        ],
