...
**Last updated:** 2026-05-03 (B2.1 merged via [#197](https://github.com/favouritekid/QLTS/pull/197) squash `df2111a9` — 12 ADMISSION_* events catalog + group + seed + `EventDefinition` extension; 103/103 post-merge. B2.2 / M-1-19a merged via [#198](https://github.com/favouritekid/QLTS/pull/198) squash `e05732c2` — `NotificationOutbox` model + migration `phase1_19a` + retire T0-4a canary; 26/26 post-merge; dev DB `phase0br01 → phase1_19a (head)`. B2.3 PR [#199](https://github.com/favouritekid/QLTS/pull/199) open 2026-05-03 on `feature/admission-b2-3` off parent `31f7e001` — `dispatch_event()` wrapper using `safe_dispatch()` for best-effort callback (PATCH-11 drift resolved) + isinstance enum guard + coverage script extended với raw-dispatch-of-outbox detector covering keyword + positional forms (bite-verified twice); 26 passed + 1 skipped wrapper; 40 passed + 1 skipped focused (2 file `test_dispatch_event_wrapper.py + test_coverage_script_raw_dispatch.py`); 169 passed + 1 skipped full notification regression; pending review/merge. B2.4 / T0-4b TODO. B2 thematic checkbox NOT ticked, reserved for B2.4 close)
...
| B2 | EventDefinition extend (`requires_outbox`, `bypass_consent_check`) + 12 ADMISSION_* enum + EVENT_CATALOG seed + `dispatch_event` wrapper + `NotificationOutbox` model + migration. **Split 4 sub-PR**: B2.1 (catalog + group + seed) → B2.2 (model + migration) → B2.3 (wrapper) → B2.4 (T0-4b worker). | BE | IN_PROGRESS (B2.1 TESTED 2026-05-03; B2.2 TESTED 2026-05-03; B2.3 CODE_DONE 2026-05-03 (PR [#199](https://github.com/favouritekid/QLTS/pull/199) open, pending review/merge); B2.4 TODO) | B2.1: [#197](https://github.com/favouritekid/QLTS/pull/197) merged 2026-05-03 squash `df2111a9`. B2.2 / M-1-19a: [#198](https://github.com/favouritekid/QLTS/pull/198) merged 2026-05-03 squash `e05732c2`. B2.3: [#199](https://github.com/favouritekid/QLTS/pull/199) open 2026-05-03 on `feature/admission-b2-3` off parent `31f7e001` (post B2.2 tracking) — pending review/merge. B2.4: TODO | B2.1 U:✓ 103/103. B2.2 U:✓ 26/26. **B2.3 U:✓ 26 passed + 1 skipped wrapper-focused (1.09s) + 14/14 coverage detector regression (0.83s) + B2.3 focused total 40 passed + 1 skipped (2 file `test_dispatch_event_wrapper.py + test_coverage_script_raw_dispatch.py`) + 169 passed + 1 skipped full notification regression (36.92s)**: 7-event outbox parametrize (insert + None + missing-dedupe-raises + no-safe-dispatch-call) + 5-event best-effort parametrize (callable + no-outbox) + callback kwargs forwarding + bypass_consent True-case via `get_event` patch + isinstance guard rejecting str + int + unknown-enum (skip if catalog complete) + API surface lock (export + keyword-only signature). Coverage detector covers keyword form `event=SystemEvents.X` AND positional form `safe_dispatch(db, SystemEvents.X, ...)` (Codex catch during amend — extracted `_extract_systemevents_from_event_arg()` helper, regression tests cover both forms + parameterized-skip + allowlist + dispatch_event-not-flagged + gap-surfacing). Bite-verified `raw-dispatch-of-outbox-event` detector twice: keyword stub on `ADMISSION_DECISION_ADMITTED` + positional stub on `ADMISSION_DECISION_REJECTED` both flagged correctly + reverted to baseline. **B2.3 contract decision (doc drift resolved)**: wrapper uses `safe_dispatch()` (no strict) trong best-effort callback because callback chạy router post-commit (no outer tx → no savepoint to protect → memory `dispatch-bundle-strict-required` doesn't apply). RISK_REVIEW PATCH-11 wording `safe_dispatch(strict=True)` là drift — `safe_dispatch()` không có `strict` param (verified line 1853-1859). Outbox path inserts `NotificationOutbox` row trong tx caller, returns None — không gọi dispatch/safe_dispatch. Coverage script exits `1` with 12 expected `no-dispatch-site` (unchanged baseline; #16 wires service callers); 0 raw-dispatch-of-outbox-event violations. | B2.4 sequential: T0-4b real `notification_outbox_drain` worker body — replaces T0-4a no-op skeleton, 3-step claim/dispatch/finalize per PLAN §3.3.f. | PLAN §3.3.d-f + RISK_REVIEW B2 |
