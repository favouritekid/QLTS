...
| T0-5 | `POST /api/v2/admin/casbin/reload` admin endpoint — **current-process scope** (chỉ reload 1 Gunicorn worker; multi-worker fleet-wide reload = restart backend per §7.2 T+3:15). `Depends(require_admin)` + `@limiter.limit(ADMIN_WRITE)`; thin router gọi `enforcer.load_policy()` + activity_service.log_activity audit + response field `scope: "current_process"`; KHÔNG touch auth_model.conf/deny/templates — B1 scope. | BE | CODE_DONE (PR open) | [#193](https://github.com/favouritekid/QLTS/pull/193) | U:✓ (9-case pytest `tests/api/test_admin_v2_casbin_reload.py`: admin success + response-shape lock (success/reloaded_at/policy_count/actor_id/**scope**) + non-negative policy_count + 4-role deny matrix (unauth 401, manager 403, officer 403, regular_user 403) + reload-failure structured 500 (monkeypatch `enforcer.load_policy` raise) + subsequent-reload-recovers resilience + endpoint-registration path lock) | (chờ self-review + merge sub-PR; KHÔNG merge vội) | RUNBOOK §3.5 + §7.2 T+3:15 |
...
| T0-5 Casbin reload endpoint shipped (current-process scope; fleet reload = restart backend §7.2 T+3:15) | CODE_DONE | T0-5 |
