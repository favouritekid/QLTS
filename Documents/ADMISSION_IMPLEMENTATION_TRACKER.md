...
**Last updated:** 2026-05-03 (B2.1 merged via [#197](https://github.com/favouritekid/QLTS/pull/197) squash `df2111a9` — 12 ADMISSION_* events catalog + group + seed + `EventDefinition` extension; 103/103 post-merge re-run on parent; coverage script intentionally red until B2.3 + #16. B2.2 / M-1-19a CODE_DONE local 2026-05-03 on `feature/admission-b2-2-m-1-19a` off parent `acdffc8e` — `NotificationOutbox` model + migration `phase1_19a` + retire T0-4a canary; 26/26 unit + alembic upgrade/downgrade/re-upgrade roundtrip PASS; pending push approval)
...
| B2 | EventDefinition extend (`requires_outbox`, `bypass_consent_check`) + 12 ADMISSION_* enum + EVENT_CATALOG seed + `dispatch_event` wrapper + `NotificationOutbox` model + migration. **Split 4 sub-PR**: B2.1 (catalog + group + seed) → B2.2 (model + migration) → B2.3 (wrapper) → B2.4 (T0-4b worker). | BE | IN_PROGRESS (B2.1 TESTED 2026-05-03; B2.2 CODE_DONE local 2026-05-03 pending push; B2.3/4 TODO) | B2.1: [#197](https://github.com/favouritekid/QLTS/pull/197) merged 2026-05-03 squash `df2111a9`. B2.2 / M-1-19a: branch `feature/admission-b2-2-m-1-19a` off parent `acdffc8e` (post B2.1 tracking) — pending push approval. B2.3-4: TODO | B2.1 U:✓ 103/103 post-merge re-run on parent `df2111a9` (34.89s): 65 B2.1 lock-in (enum + dataclass + cross-file parity + outbox/bypass matrix per PLAN §3.3.d) + 34 notification_contract (incl. P2 fix `_PENDING_DISPATCH_EVENTS` split + 2 lock tests `disjoint` + `locked-to-12-set`, bite-verified) + 4 event_groups_api regression. Coverage script exits `1` with 12 expected `no-dispatch-site` (admission_*); will green after B2.3 + #16. **B2.2 U:✓ 26/26 (3.50s)**: 14 model+migration parity (importable + 10-col shape + BIGSERIAL + JSONB + named UNIQUE + 2 partial WHERE indexes + revision chain + head no-children + ORM ⇄ migration constants) + 10 outbox skeleton regression (canary `test_models_package_still_lacks_notification_outbox` retired; 2 AST guards on skeleton + tasks `__init__` intact) + 2 celery_task_registry regression. **B2.2 M:✓ live alembic roundtrip dev DB**: `phase0br01 → phase1_19a (head)` upgrade ✓ table+10 col+3 idx+1 UNIQUE per PLAN §3.3.e/f; `downgrade -1 → phase0br01` ✓ table dropped clean (`Did not find any relation`); re-`upgrade head → phase1_19a` ✓ idempotent (`to_regclass = t`). | B2.3/4 sequential after B2.2 merge: B2.3 (`dispatch_event` wrapper) → B2.4 (T0-4b real worker, replaces T0-4a no-op skeleton) | PLAN §3.3.d-f + RISK_REVIEW B2 |
...
| M-1-19a | `phase1_19a_create_notification_outbox` (10 cột + named UNIQUE `uq_notification_outbox_idempotency_key` + 2 partial WHERE idx `ix_outbox_pending(created_at)` / `ix_outbox_claim(claimed_until)` cùng predicate `dispatched_at IS NULL`; idempotent guards `table_exists` + `index_exists`; downgrade short-circuits if table gone, drops indexes then table). `down_revision='phase0br01'`, ship cùng B2.2 sub-PR. | BE | CODE_DONE local 2026-05-03 (pending push) | branch `feature/admission-b2-2-m-1-19a` off parent `acdffc8e` | M:✓ live alembic roundtrip dev DB `phase0br01 → phase1_19a` upgrade ✓ table+10col+3idx+1UNIQUE; `downgrade -1` ✓ relation gone; re-upgrade ✓ idempotent. U:✓ ORM ⇄ migration parity 14-test (column shape + types + UNIQUE + 2 partial WHERE + revision chain + head no-children + name constants). Live HTTP smoke deferred staging clone D12-D14. | B2 ship cùng PR (NotificationOutbox model + retire T0-4a canary) | PLAN §4 P1 #19a |
