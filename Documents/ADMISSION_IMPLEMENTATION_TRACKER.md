...
**Last updated:** 2026-05-03 (B2.4 / T0-4b merged via [#200](https://github.com/favouritekid/QLTS/pull/200) squash `737eb1bc` — real claim/dispatch/finalize outbox worker replaces T0-4a skeleton. B1 Casbin deny-first CODE_DONE local 2026-05-03 on `feature/admission-b1` off parent `88c19c16` — atomic single-PR per Strategy A: 4-field auth_model.conf + canonical Casbin allow-and-deny effect + 6 accountant deny rules (PLAN §3.3.b lines 1411-1415, waitlist-* expanded) + RUN_CASBIN_LOAD_ON_STARTUP cold-cutover gate + migration `phase1_19b_backfill_casbin_eft_and_seed_deny_rules` (210 row v3=NULL → v3='allow' + 6 INSERT, idempotent predicate-bounded) + RUNBOOK §7.2 patch (3rd flag T+1:00, T+3:15, T+24h). 39 B1 lock-in tests (incl. boot-gate-bypass added per Codex review patch) + 9 existing Casbin tracking regression = 48 focused (22.79s); 225 passed + 1 skipped full B1+B2 regression 13 files (87.76s). Alembic dev-DB roundtrip clean. **Diamond inheritance gotcha documented**: admin inherits accountant via diamond → deny-first propagates → admin denied on v2 routes; PLAN line 1407 admin-allow on v2 deferred follow-up alongside #15. Pending review/merge. #16 remains blocked until B1 + B2 are both TESTED on parent.)
**Current sprint focus:** Phase 1 Code — B1 Casbin deny-first next; D2/D3 cutover gates continue in parallel and do not block dev
...
| M-1-casbin | `phase1_19b_backfill_casbin_eft_and_seed_deny_rules` — UPDATE casbin_rule SET v3='allow' WHERE ptype='p' AND v3 IS NULL (idempotent predicate-bounded) + 6 INSERT deny rules accountant (WHERE NOT EXISTS guard, template_id='_system_b1_deny' marker, asyncpg-safe `CAST(:p AS varchar)` casts to avoid AmbiguousParameterError on parameter reuse). `down_revision='phase1_19a'`. Downgrade reverses both: DELETE 6 deny rows by exact match, UPDATE v3=NULL WHERE v3='allow'. | BE | CODE_DONE local 2026-05-03 (pending push) | shipped trong B1 PR — branch `feature/admission-b1` | M:✓ live alembic upgrade/downgrade/re-upgrade roundtrip on dev DB; idempotent re-apply no-op; 7 lock-in tests (revision contract, predicate-guard regex, asyncpg cast count, 6-rule explicit set, template_id marker). | B1 ship cùng PR — không tách riêng (preflight chứng minh B1.1 split shape không khả thi). | PLAN §4 P1 + RISK_REVIEW B1 |
