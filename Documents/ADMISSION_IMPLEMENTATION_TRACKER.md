...
**Last updated:** 2026-05-03 (B2.1 merged via [#197](https://github.com/favouritekid/QLTS/pull/197) squash `df2111a9` — 12 ADMISSION_* events catalog + group + seed + `EventDefinition` extension; 103/103 post-merge re-run on parent; coverage script intentionally red until B2.3 + #16. B2.2 / M-1-19a merged via [#198](https://github.com/favouritekid/QLTS/pull/198) squash `e05732c2` — `NotificationOutbox` model + migration `phase1_19a` + retire T0-4a canary; 26/26 post-merge re-run on parent `e05732c2`; dev DB advanced `phase0br01 → phase1_19a (head)`; B2 thematic checkbox NOT ticked, reserved cho B2.4/T0-4b close)
...
| B2 | EventDefinition extend (`requires_outbox`, `bypass_consent_check`) + 12 ADMISSION_* enum + EVENT_CATALOG seed + `dispatch_event` wrapper + `NotificationOutbox` model + migration. **Split 4 sub-PR**: B2.1 (catalog + group + seed) → B2.2 (model + migration) → B2.3 (wrapper) → B2.4 (T0-4b worker). | BE | IN_PROGRESS (B2.1 TESTED 2026-05-03; B2.2 TESTED 2026-05-03; B2.3/4 TODO) | B2.1: [#197](https://github.com/favouritekid/QLTS/pull/197) merged 2026-05-03 squash `df2111a9`. B2.2 / M-1-19a: [#198](https://github.com/favouritekid/QLTS/pull/198) merged 2026-05-03 squash `e05732c2`. B2.3-4: TODO | B2.1 U:✓ 103/103. **B2.2 U:✓ 26/26 post-merge re-run on parent `e05732c2` (4.29s)**: 14 model+migration parity (importable + 10-col shape + BIGSERIAL + JSONB + named UNIQUE + 2 partial WHERE indexes + revision chain + head no-children + ORM ⇄ migration constants) + 10 outbox skeleton regression (canary `test_models_package_still_lacks_notification_outbox` retired; 2 AST guards on skeleton + tasks `__init__` intact) + 2 celery_task_registry regression. **B2.2 M:✓ live alembic upgrade on parent**: dev DB advanced `phase0br01 → phase1_19a (head)` post-merge; pre-merge roundtrip evidence (downgrade + re-upgrade idempotent) on local branch. Coverage script exits `1` with 12 expected `no-dispatch-site` (admission_*); will green after B2.3 + #16. | B2.3/4 sequential: B2.3 (`dispatch_event` wrapper, branch `feature/admission-b2-3` off parent `e05732c2`) → B2.4 (T0-4b real worker, replaces T0-4a no-op skeleton) | PLAN §3.3.d-f + RISK_REVIEW B2 |
