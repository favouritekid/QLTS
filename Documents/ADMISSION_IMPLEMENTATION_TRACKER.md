...
| T0-4a | Celery beat `dispatch_pending_outbox` **skeleton task** (no-op safe registration: function defined, beat schedule registered, log "outbox not yet active" + return early — KHÔNG insert/dispatch). Kịch bản: B2/M-1-19a chưa ship, beat task vẫn registered nhưng KHÔNG crash worker. | BE | CODE_DONE (PR open) | [#192](https://github.com/favouritekid/QLTS/pull/192) | U:✓ (13-case pytest `tests/unit/test_outbox_skeleton.py` (11) + `test_celery_task_registry.py` (2) re-run: beat-schedule cadence + static `conf.include` config check + **subprocess cold-import regression test** (fresh `python -c "from app.celery_app import celery_app"` without finalize/import_default_modules — bite-verified) + post-import sanity + result-shape stability + AST-based no-NotificationOutbox-import guard for skeleton module + tasks/__init__.py + models package gap assertion + autodiscover smoke + zero-arg signature contract) | (chờ self-review + merge sub-PR; KHÔNG merge vội) | RUNBOOK §3.5 + PLAN §3.3.e |
...
| T0-4a dispatch_pending_outbox skeleton (no-op safe) | CODE_DONE | T0-4a |
