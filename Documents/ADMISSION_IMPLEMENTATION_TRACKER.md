...
| T0-5 | `POST /api/v2/admin/casbin/reload` admin endpoint (admin-only via `require_admin` dep; thin router gọi `enforcer.load_policy()` + activity_service.log_activity audit; KHÔNG touch auth_model.conf/deny/templates — B1 scope) | BE | CODE_DONE (commit local, branch chưa push) | feature/admission-t0-5 | U:✓ (9-case pytest `tests/api/test_admin_v2_casbin_reload.py`: admin success + response-shape lock (success/reloaded_at/policy_count/actor_id) + non-negative policy_count + 4-role deny matrix (unauth 401, manager 403, officer 403, regular_user 403) + reload-failure structured 500 (monkeypatch `enforcer.load_policy` raise) + subsequent-reload-recovers resilience + endpoint-registration path lock) | (chờ push approval + sub-PR target `feat/admission-full-cutover`) | RUNBOOK §3.5 |
...
| T0-5 Casbin reload endpoint shipped | CODE_DONE | T0-5 |
