...
| T0-3 | Nginx admission block env-driven `NGINX_ADMISSION_FROZEN` (regex `^/api/(admissions\|admission-config\|public/admissions)(/.*)?$`; envsubst at deploy time; pair với T0-2 backend middleware) | Ops | CODE_DONE (commit local, branch chưa push) | feature/admission-t0-3 | U:✓ (32-case `bash scripts/test_nginx_admission_freeze.sh`: 15 render-substitution + 3 nginx -t syntax in Docker + 14 regex URI match including 4 lookalike rejection); live HTTP smoke deferred staging (cần SSL certs + live upstream) | (chờ push approval + sub-PR target `feat/admission-full-cutover`) | RUNBOOK §3.5 |
