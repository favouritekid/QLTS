...
| T0-3 | Nginx admission block env-driven `NGINX_ADMISSION_FROZEN` (regex `^/api/(admissions\|admission-config\|public/admissions)(/.*)?$`; envsubst at deploy time; pair với T0-2 backend middleware) | Ops | CODE_DONE (PR open) | [#191](https://github.com/favouritekid/QLTS/pull/191) | U:✓ (32-case `bash scripts/test_nginx_admission_freeze.sh`: 15 render-substitution + 3 nginx -t syntax in Docker + 14 regex URI match including 4 lookalike rejection); live HTTP smoke deferred staging (cần SSL certs + live upstream) | (chờ self-review + merge sub-PR; KHÔNG merge vội) | RUNBOOK §3.5 |
