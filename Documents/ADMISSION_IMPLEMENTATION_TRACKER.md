...
| M-1-19d | `phase1_19d_register_celery_beat_archive_task` (renamed từ spec 19c; marker migration + paired Celery code: task body archive_outbox_dispatched_task weekly 90d + beat entry; pattern B-i static dict per `app/celery_app.py:118-255`); ship-order Wave 5-B (AFTER M-1-17 archive table) | BE | TODO | | M:- I:- | M-1-17 (`_archived_notification_outbox` table phải ship trước archive task body); T0-4 dispatch_pending_outbox đã ship cùng; M-1-19c TESTED ✓ | PLAN §4 P1 #19d (post-cascade) — ship-order reorder chốt 2026-05-05 |
| M-1-19e | `phase1_19e_seed_notification_rules` (renamed từ spec 19d; channel routing rule rows per audience từng event — Zalo/email/in-app); ship-order Wave 5-C (last in Wave 5) | BE | TODO | | M:- I:- | M-1-19a + M-1-19c TESTED ✓ | PLAN §4 P1 #19e (post-cascade) |
