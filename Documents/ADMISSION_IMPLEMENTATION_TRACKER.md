...
**Last updated:** 2026-05-03 (B2.1 merged via [#197](https://github.com/favouritekid/QLTS/pull/197) squash `df2111a9` — 12 ADMISSION_* events catalog + group + seed + `EventDefinition` extension; 103/103 post-merge. B2.2 / M-1-19a merged via [#198](https://github.com/favouritekid/QLTS/pull/198) squash `e05732c2` — `NotificationOutbox` model + migration `phase1_19a` + retire T0-4a canary; 26/26 post-merge; dev DB `phase0br01 → phase1_19a (head)`. B2.3 merged via [#199](https://github.com/favouritekid/QLTS/pull/199) squash `f2f0d62b` — `dispatch_event()` wrapper using `safe_dispatch()` for best-effort callback (PATCH-11 drift resolved) + isinstance enum guard + coverage script extended với raw-dispatch-of-outbox detector covering keyword + positional forms (bite-verified twice); post-merge full notification regression 169 passed + 1 skipped on parent `f2f0d62b` (37.94s); coverage invariant 0 raw-dispatch-of-outbox-event violations + 12 expected no-dispatch-site; alembic head unchanged `phase1_19a`. B2.4 / T0-4b PR [#200](https://github.com/favouritekid/QLTS/pull/200) OPEN/CLEAN on `feature/admission-b2-4`, head `e806597a` off parent `87685ff4` — real claim/dispatch/finalize worker replaces T0-4a skeleton; 10 worker behavior tests on real DB incl. 2 added on Codex review patch (retry success clears stale last_error + adaptive lease per actual claim count via SQL CTE COUNT, 1-row claim ~5s lease not 500s); 177 passed + 1 skipped full notification regression (58.62s); coverage script unchanged (12 expected no-dispatch-site + 0 raw-dispatch-of-outbox); alembic head unchanged; pending review/merge. B2 thematic checkbox NOT ticked, will tick after B2.4 merge closes the wave)
...
| T0-4b | Celery beat `dispatch_pending_outbox` **real worker wiring** (3-step claim/dispatch/finalize, query NotificationOutbox table, dispatch SystemEvents enum). Replace skeleton T0-4a. | BE | CODE_DONE 2026-05-03 (PR #200 open, pending review/merge) | [#200](https://github.com/favouritekid/QLTS/pull/200) open 2026-05-03, head `e806597a` on `feature/admission-b2-4` off parent `87685ff4` (post B2.3 tracking). Ships co-located with B2.4. | U:✓ 10 worker behavior tests on real DB (empty queue / dispatched_at marking / failure → last_error + claim release / expired-lease re-claim / active-lease blocks re-claim / DLQ at MAX_ATTEMPTS=5 / mixed batch isolation / post-commit callback awaited / retry success clears stale last_error / single-row lease ~PER_ROW_TIMEOUT_SECONDS not BATCH_LIMIT × PER_ROW_TIMEOUT_SECONDS). 8 retained registration/AST guards (test_outbox_skeleton.py renamed conceptually — 2 skeleton-specific tests retired: `test_skeleton_module_does_not_reference_notification_outbox_in_code` + `test_skeleton_call_returns_stable_no_op_shape`). Coverage script invariant: exit `1` with 12 expected `no-dispatch-site` + 0 raw-dispatch-of-outbox-event (worker module allowlisted per B2.3 detector). | DONE chờ staging clone D12-D14 smoke (apply T0-4b → restart `celery-worker` + `celery-beat` → tail logs cho `dispatch_pending_outbox: queue empty` mỗi 30s khi outbox rỗng / `dispatch_pending_outbox: tick complete claimed=N dispatched=N failed=N` khi có rows; chèn 1 outbox row test → quan sát lifecycle claimed_at → dispatched_at within ~30s). | RUNBOOK §3.5 + PLAN §3.3.e/f |
...
| B2 | EventDefinition extend (`requires_outbox`, `bypass_consent_check`) + 12 ADMISSION_* enum + EVENT_CATALOG seed + `dispatch_event` wrapper + `NotificationOutbox` model + migration. **Split 4 sub-PR**: B2.1 (catalog + group + seed) → B2.2 (model + migration) → B2.3 (wrapper) → B2.4 (T0-4b worker). | BE | IN_PROGRESS (B2.1 TESTED 2026-05-03; B2.2 TESTED 2026-05-03; B2.3 TESTED 2026-05-03; B2.4 / T0-4b CODE_DONE 2026-05-03 via PR [#200](https://github.com/favouritekid/QLTS/pull/200) open, pending review/merge) | B2.1: [#197](https://github.com/favouritekid/QLTS/pull/197) merged 2026-05-03 squash `df2111a9`. B2.2 / M-1-19a: [#198](https://github.com/favouritekid/QLTS/pull/198) merged 2026-05-03 squash `e05732c2`. B2.3: [#199](https://github.com/favouritekid/QLTS/pull/199) merged 2026-05-03 squash `f2f0d62b`. B2.4 / T0-4b: [#200](https://github.com/favouritekid/QLTS/pull/200) open 2026-05-03, head `e806597a` on `feature/admission-b2-4` off parent `87685ff4` (post B2.3 tracking) — pending review/merge. | B2.1 U:✓ 103/103. B2.2 U:✓ 26/26. **B2.3 U:✓ post-merge full notification regression 169 passed + 1 skipped on parent `f2f0d62b` (37.94s)**: includes wrapper-focused 26 passed + 1 skipped, coverage detector regression 14/14, outbox model/skeleton/notification_contract/B2.1/celery/event_groups regressions. Coverage detector covers keyword form `event=SystemEvents.X` AND positional form `safe_dispatch(db, SystemEvents.X, ...)` (Codex catch during amend — extracted `_extract_systemevents_from_event_arg()` helper, regression tests cover both forms + parameterized-skip + allowlist + dispatch_event-not-flagged + gap-surfacing). Bite-verified `raw-dispatch-of-outbox-event` detector twice before merge: keyword stub on `ADMISSION_DECISION_ADMITTED` + positional stub on `ADMISSION_DECISION_REJECTED` both flagged correctly + reverted to baseline. **B2.3 contract decision (doc drift resolved)**: wrapper uses `safe_dispatch()` (no strict) trong best-effort callback because callback chạy router post-commit (no outer tx → no savepoint to protect → memory `dispatch-bundle-strict-required` doesn't apply). RISK_REVIEW PATCH-11 wording `safe_dispatch(strict=True)` là drift — `safe_dispatch()` không có `strict` param (verified line 1853-1859). Outbox path inserts `NotificationOutbox` row trong tx caller, returns None — không gọi dispatch/safe_dispatch. Coverage invariant post-merge: 0 raw-dispatch-of-outbox-event violations, 12 expected `no-dispatch-site` (unchanged baseline; #16 wires service callers), 7 outbox events tracked. Alembic head unchanged `phase1_19a`. **B2.4 / T0-4b U:✓ 10 worker behavior tests + 8 retained registration/AST guards (2 skeleton-specific tests retired); full notification regression 177 passed + 1 skipped (58.62s)** — empty-queue / dispatched-row / failure / expired-lease / active-lease / DLQ-cap / mixed-batch / post-commit-callback / retry-clears-last-error / single-row-lease-is-per-row-seconds. Two patches landed pre-push (Codex review): Step-3 ok branch clears stale `last_error` on successful retry; Step-1 lease window adaptive per actual claim count via inline `LEAST((SELECT COUNT(*) FROM candidates) * PER_ROW_TIMEOUT_SECONDS, CLAIM_TIMEOUT_CAP_SECONDS)` (1-row claim gets ~5s lease, not the prior 500s naive shape). Worker module is allowlisted in coverage script (raw `dispatch()` + `safe_dispatch()` allowed because IT IS the dispatcher for outbox events). Coverage invariant unchanged: exit `1` with 12 `no-dispatch-site` + 0 raw-dispatch-of-outbox-event. Alembic head still `phase1_19a`. After B2.4 merge: tick `[ ] **B2**` checkbox on issue #183 (entire B2 wave + T0-4b closed). | After B2.4 merge: B2 wave complete; **NEXT = B1 Casbin deny-first**. **#16 remains blocked** until B1 + B2 are both TESTED; then proceed #15 / #16 per sequence. The 12 expected `no-dispatch-site` gaps stay until #16 wires `state_service.transition()` through `dispatch_event()`. | PLAN §3.3.d-f + RISK_REVIEW B2 |
