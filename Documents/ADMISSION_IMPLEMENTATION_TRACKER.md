...
| M-P0b | `phase0b_relax_applied_rules_immutability_for_payment_keys` (revision `phase0br01`, down `phase0sg01`); CREATE OR REPLACE FUNCTION `prevent_applied_rules_update` với per-key classifier; whitelist **5 key** add/update (`fee_status` + `fee_paid_at` + `fee_payment_data` + `fee_calculated_at` + `fee_invoice_id`) — `fee_status` thêm trong PR review (PLAN listed 4 miss); deletion guard reject xóa whitelisted key; downgrade restore v1 strict literal-for-literal từ `b5c6d7e8f9a0`. PLAN §3.4 lines 2516-2557 patched cùng PR (drift fix verified). | Migration | BE | CODE_DONE (commit local, branch chưa push) | feature/admission-m-p0b | M:✓ live alembic dev DB roundtrip + 6-scenario psql behavior matrix (5 allowed keys add/update PASS + 4 rejection: non-allowed min_gpa/admission_path_id + mixed allowed+non-allowed + deletion of whitelisted; no-op fast-path PASS) + 14/14 existing fee tests PASS (`tests/services/test_admission_application_fee.py`) + U:✓ 4-case source contract (revision chain + 5-key whitelist + per-key classifier markers + downgrade v1 literal) | (chờ push approval + sub-PR target `feat/admission-full-cutover`) | PLAN §4 Phase 0b |
