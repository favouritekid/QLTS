...
| M-P0b | `phase0b_relax_applied_rules_immutability_for_payment_keys` (revision `phase0br01`, down `phase0sg01`); CREATE OR REPLACE FUNCTION `prevent_applied_rules_update` với per-key classifier; whitelist **5 key** add/update (`fee_status` + `fee_paid_at` + `fee_payment_data` + `fee_calculated_at` + `fee_invoice_id`) — `fee_status` thêm trong PR review (PLAN listed 4 miss); deletion guard reject xóa whitelisted key; downgrade restore v1 strict literal-for-literal từ `b5c6d7e8f9a0`. PLAN §3.4 lines 2516-2557 patched cùng PR (drift fix verified). | Migration | BE | CODE_DONE (PR open) | [#196](https://github.com/favouritekid/QLTS/pull/196) | M:✓ live alembic dev DB roundtrip + 6-scenario psql behavior matrix + 14/14 existing fee tests PASS + U:✓ 4-case source contract | (chờ self-review + merge sub-PR; KHÔNG merge vội) | PLAN §4 Phase 0b |
