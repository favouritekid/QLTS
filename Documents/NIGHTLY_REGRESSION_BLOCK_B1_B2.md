...
## Trạng thái thực hiện — cập nhật 13-09-2026

Hai phạm vi sửa ở trên **đã code xong tại máy** (chưa push, chưa PR, chưa
deploy). Mục này ghi lại những gì đã ĐO được từ lúc viết bản đầu, để người
đọc sau không phải suy lại.

### B1 — đã vá, và blast radius production = 0

Bản vá không chỉ "giữ `kv_resolved` qua freeze". Nó thêm một cổng xuất xứ
(`priority_service.verify_manual_override_provenance`) đòi bằng chứng độc lập
là hàng `priority_audit_log` với `action_type='kv_manual_override'`, kèm vân
tay đầu vào KV. **Đây là thay đổi tương thích có chủ ý**: mọi bản ghi override
cũ thiếu vân tay sẽ bị từ chối. Vì vậy phải đo trước khi deploy, không được
coi là "chỉ sửa lỗi giữ KV".

Đo trực tiếp trên production (read-only, thuần tổng hợp, không đọc cột nhân
thân nào), 13-09-2026, prod ở `7e67fe46`:

| Phép đo | Kết quả |
|---|---|
| `admission_profile` tổng | 969 |
| snapshot rỗng (`'{}'::jsonb`) | 63 |
| có khoá `manual_override_by` | **0** |
| `rule_applied = 'manual_override'` | **0** |
| `area_resolution_basis = 'manual_override'` | **0** |
| `priority_audit_log` tổng số hàng | **0** |
| `action_type = 'kv_manual_override'` | **0** |

⇒ **Blast radius = 0 hồ sơ.** Cổng xuất xứ mới không thể từ chối thứ gì đang
tồn tại, vì hiện không hồ sơ nào khai override KV. Con số này phải ĐO LẠI nếu
deploy diễn ra sau khi có người dùng đường override.

### B2 — đã vá; trên production là no-op ở đường thành công

Bản vá tra theo `code='NOT_CONTACTED'` qua `StatusHelper.get_initial_status`,
fail-closed bằng 503 khi hàng chuẩn thiếu hoặc sai cấu hình.

Đo trực tiếp trên production cùng lượt:

| Phép đo | Kết quả |
|---|---|
| số hàng `code='NOT_CONTACTED'` | 1 (`sts00`) |
| `is_final` | `false` ✓ |
| `is_universal` | `false` ✓ |
| `stage_id` | `stg01` (khác NULL) ✓ |
| `legacy_status` | `'new'` |
| `lead` tổng | 3 535 |
| `lead.consultation_status_id IS NULL` | 1 |

⇒ `check_initial_status_row` **ĐẠT** trên production, nên bản vá là **no-op ở
đường thành công**. Nó chỉ đổi hành vi trên CSDL mà hàng chuẩn thiếu/sai cấu
hình — đúng hoàn cảnh của CI — nơi trước đây lead được tạo với trạng thái
NULL trong im lặng, nay là 503.

⚠️ Điều kiện `stage_id` là **KHÁC NULL**. Bản đầu của tài liệu này từng ghi
`stage_id IS NULL` là điều kiện hợp lệ — sai, mã thực tế đòi ngược lại.

1 lead đang có `consultation_status_id IS NULL` là **cặn có sẵn**, bản vá
không đụng tới: nhánh `and current_status_id`
(`lead_service.py:2165-2166`) giữ nguyên từng chữ, nên lead ấy hành xử y như
trước. Bản vá chặn lead NULL MỚI, không dọn lead NULL cũ.

### Vẫn đúng: không gộp vào PR #629

Mục "Vì sao không sửa chung với PR #629" ở trên giữ nguyên hiệu lực. Các lát
đã được tách sẵn ở máy theo đúng ranh giới ấy, và đo được là gần như rời hẳn
nhau — xem báo cáo bàn giao của đợt.
