...
test.describe("Lead Management Workflow", () => {
  ...
  test("Lead CRUD + Consultation + Status progression", async ({ page }) => {
    ...
    await test.step("Admin login + discover config", async () => {
      ...
      const ungVien = (await nextResp.json()) as Array<{
        ...
        code?: string | null;
        is_final?: boolean;
        selectable_mode?: string | null;
      }>;
      // Ưu tiên `selectable_mode === "user"`: đó là tập trạng thái mà NGƯỜI
      // DÙNG được phép chọn, đúng thứ một cuộc tư vấn thật sẽ đặt. Trạng thái
      // `system`/`auto` do máy đặt, chọn nhầm thì ca đỏ vì lý do chẳng liên quan.
      const keTiep =
        ungVien.find(
          (s) => s.id !== initialStatusId && !s.is_final && s.selectable_mode === "user"
        ) ?? ungVien.find((s) => s.id !== initialStatusId && !s.is_final);
      expect(
        keTiep,
        `FSM phải cho ít nhất một trạng thái KHÔNG-final đi tới được từ ` +
          `${initialStatusId}; nhận được ${ungVien.length} ứng viên: ` +
          `${ungVien
            .map((s) => `${s.id}[${s.selectable_mode ?? "?"}]${s.is_final ? "(final)" : ""}`)
            .join(",")}`
      ).toBeTruthy();
  ...
  test("IDOR: officer cannot access unassigned lead + FSM status patch + officer action + audit logs", async ({ page }) => {
    ...
    await test.step("Add consultation to lead4 (set initial status)", async () => {
      ...
      // `status_updated` phải là **false** — và điều đó là DẤU HIỆU BẢN VÁ B2
      // CÓ TÁC DỤNG, không phải hồi quy.
      //
      // Trước bản vá: `StatusHelper.get_initial_status` dò CỘT
      // `legacy_status='new'`, mà trên mọi CSDL mới không hàng nào thoả ⇒
      // `create_lead` ghi lead với `consultation_status_id = NULL`. Khi ấy
      // POST consultation ở `initialStatusId` là một THAY ĐỔI THẬT
      // (NULL → sts00) nên `status_updated = true`.
      //
      // Tệ hơn thế: `lead_service.py:2163-2166` gác khối validate FSM + phase
      // guard bằng `and current_status_id`, nên lead status NULL làm **toàn bộ
      // máy trạng thái bị bỏ qua** — `POST /consultations` nhận BẤT KỲ
      // `status_id` nào. Ca này từng xanh nhờ đúng lối hỏng ấy.
      //
      // Sau bản vá: lead mới đứng ở `sts00` ngay lúc tạo, nên đặt lại đúng
      // trạng thái đang có là **no-op hợp lệ**. Hợp đồng đọc từ
      // `lead_service.py:2395` — `status_updated = will_be_latest and
      // (old_state != new_state)` với `old/new` lấy từ `_get_current_lead_state`
      // (5 trường): cả 5 không đổi ⇒ `false`. Bản ghi consultation VẪN được
      // tạo (201) vì đó là lịch sử liên hệ, không phải chuyển trạng thái.
      expect(
        body.status_updated,
        `Lead đã ở ${initialStatusId} ngay từ lúc tạo nên đặt lại đúng trạng thái ` +
          `ấy phải là no-op. status_updated=true nghĩa là lead KHÔNG mang trạng ` +
          `thái khởi tạo — tức bản vá B2 chưa có hiệu lực. ` +
          `terminal_guard_reason=${body.terminal_guard_reason}`
      ).toBe(false);
  ...
  test("Import leads from CSV file", async ({ page }) => {
    ...
    await test.step("Import CSV with missing phone → row-level error", async () => {
      ...
      // Phép kiểm cũ `status()===200 || status()===400` là phép kiểm GỘP, và
      // nó đã CHE đúng lỗi mà batch này đi tìm: import trên CSDL mới trả
      // `400 "System configuration error: Initial lead status not found."`
      // — một lỗi CẤU HÌNH TOÀN CỤC — mà vẫn lọt qua vì 400 nằm trong tập
      // được chấp nhận. Ca dòng lỗi (thiếu số điện thoại) phải báo ở CẤP DÒNG
      // trong thân phản hồi, không phải bằng mã trạng thái của cả request.
      //
      // Nay lỗi cấu hình là 503 `INITIAL_LEAD_STATUS_NOT_CONFIGURED` chứ không
      // còn là 400, nên siết về đúng 200 là an toàn và đúng nghĩa.
      await expectOk(resp, "POST /api/leads/import (dòng thiếu số điện thoại)", [200]);
      if (resp.ok()) {
        const body = await resp.json();
        console.log(
...
test.describe("Lead terminal hard block — độc lập", () => {
  ...
  test("Terminal lead chặn consultation mới bằng 400", async ({ playwright }) => {
    ...
    // Phase theo từng status — cần để chọn ĐÚNG nhánh hợp đồng ở bước cuối
    // (`check_terminal_status_guard` phân biệt `phase="enrolled"` với phần còn lại).
    const phaseTheoStatus = new Map<string, string>();
    ...
    await test.step("Khám phá pipeline + tạo lead của riêng ca này", async () => {
      ...
      for (const st of statuses) phaseTheoStatus.set(st.id, st.phase);
      expect(
        finalIds.size,
    ...
    await test.step("Consultation mới trên lead terminal phải 400", async () => {
      ...
      // HỢP ĐỒNG THẬT có HAI nhánh, không phải một —
      // `lead_service.check_terminal_status_guard` (:186-199) nói rõ:
      //   `is_final=true` VÀ `phase="enrolled"`  → HARD BLOCK (từ chối hẳn)
      //   `is_final=true` VÀ `phase!="enrolled"` → SOFT BLOCK (VẪN ghi nhận
      //                                            cuộc liên hệ, chỉ KHÔNG
      //                                            đổi trạng thái lead)
      //
      // Bản trước khẳng định "mọi lead is_final phải 400" — SAI, và đã đỏ đúng
      // ở đây: FSM đưa lead tới `sts20 CONSULT_GIVEUP`
      // (`phase='consultation'`, `is_final=true`) nên đó là nhánh SOFT, và
      // backend trả 201 hoàn toàn đúng luật. Một assertion mạnh hơn hợp đồng
      // không phải là guard chặt hơn — nó là guard SAI.
      const phaseCuaTerminal = phaseTheoStatus.get(afterStatusId);
      expect(
        phaseCuaTerminal,
        `Không tra được phase của ${afterStatusId} trong /api/pipeline/all`
      ).toBeTruthy();

      if (phaseCuaTerminal === "enrolled") {
        expect(
          blockResp.status(),
          `Lead terminal ${afterStatusId} (phase=enrolled) phải HARD BLOCK bằng 400. ` +
        ...
        console.log(
          `HARD block xác nhận: ${afterStatusId} phase=enrolled → 400 — ` +
            safeBody({ detail: blockErr.detail })
        );
      } else {
        // SOFT BLOCK: cuộc liên hệ VẪN được ghi (đó là lịch sử, không phải
        // chuyển trạng thái), nhưng trạng thái lead KHÔNG được đổi. Phải kiểm
        // CẢ HAI, nếu không ca này xanh cả khi guard bị gỡ hẳn.
        expect(
          blockResp.status(),
          `Lead terminal ${afterStatusId} (phase=${phaseCuaTerminal}) là nhánh SOFT ` +
            `nên consultation vẫn được ghi. ` +
            summarizeApiError(blockResp.status(), blockText)
        ).toBe(201);
        const softBody = JSON.parse(blockText) as {
          status_updated?: boolean;
          terminal_guard_reason?: string | null;
        };
        expect(
          softBody.status_updated,
          `SOFT block: lead ĐANG ở trạng thái cuối ${afterStatusId} nên trạng thái ` +
            `KHÔNG được đổi. status_updated=true nghĩa là guard đã bị bỏ qua.`
        ).toBe(false);
        expect(
          softBody.terminal_guard_reason,
          `SOFT block phải nêu lý do để người đọc biết vì sao trạng thái không đổi`
        ).toBeTruthy();

        // Đo ở NGUỒN CHUẨN: trạng thái thật trên lead, không chỉ trên thân
        // phản hồi của chính lệnh vừa ghi.
        const lai = await admin.ctx.get(`${API_URL}/api/leads/${leadId}`);
        await expectOk(lai, `GET lead #${leadId} sau SOFT block`, [200]);
        expect(
          (await lai.json()).consultation_status_id,
          `Trạng thái lead phải GIỮ NGUYÊN ${afterStatusId} sau SOFT block`
        ).toBe(afterStatusId);
        console.log(
          `SOFT block xác nhận: ${afterStatusId} phase=${phaseCuaTerminal} → 201, ` +
            `status_updated=false, trạng thái giữ nguyên`
        );
      }
    });
  });
