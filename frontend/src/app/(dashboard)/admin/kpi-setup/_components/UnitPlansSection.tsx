...
import { Eye, Loader2, Pencil, Plus, RotateCcw } from "lucide-react";
...
export function UnitPlansSection({
  ...
  const previewUnitId = dialogMode?.unitId ?? null;
  const weightsKey = seasonalWeights.join(",");

  useEffect(() => {
    if (!previewUnitId || annualTarget < 1) return;
  ...
  }, [annualTarget, slaTarget, responseTime, weightsKey, previewUnitId, fiscalYear]);
  ...
  // Memoize the units table so dialog-form state changes don't re-render it
  const unitsTable = useMemo(
    ...
    [units, isAdmin, openCreateDialog, openEditDialog],
  );
                ...
                {previewMut.data && (
                  ...
                  <div className="max-h-[260px] overflow-auto rounded border">
                    <Table className="text-xs">
                      <TableHeader>
                        <TableRow>
                          <TableHead className="sticky top-0 w-12 bg-background">
                            Tháng
                          </TableHead>
